...
- **ChangeRadar**：读取同一 source identity 的两个 immutable DemoReviewed revisions；管理员面板显式确认后执行固定 `M10 → M00 durable evidence → owning M70` publication，public JSON/Atom 与重启恢复读取同一 durable item；
...
命令只接受 loopback bind。运行状态默认写入 `$XDG_STATE_HOME/ustc-campus-agent/three-plugin-mvp`（未设置时为 `~/.local/state/ustc-campus-agent/three-plugin-mvp`）；可用 `USTC_AGENTD_BIND` 与 `USTC_AGENTD_STATE_DIR` 覆盖。state directory 必须是当前用户拥有、模式 `0700` 的真实目录；launcher 不会静默 chmod/修复不安全路径。停止并以同一命令、同一 state directory 重启，会读回 Affairs records/idempotency、`m00-sessions.json`、administrator control evidence、checked Affairs/ChangeRadar publication recovery records 以及 Opportunity active profile/tombstone。Publication state 与 control evidence 是从 idempotency path 确定性派生的 owner-only sibling files；ChangeRadar JSON/Atom 都读取该 durable product state，读取不会触发管理员 write。Market activation 和 ChangeRadar source baseline 仍由同一 checkout 中的 reviewed declarative fixtures 重建并重新校验。`m00-sessions.json` 是模式 `0600`、event-history-only 的 B4a read authority，不是正式 SSO 或 durable session lifecycle mutation。

M00 的 bounded B4 interface/fake batch 现包含 B4a durable current-session read authority 与 B4b stable redacted control-event/error + append-once journal ports。Affairs 与 ChangeRadar 的 bounded B5 compositions 分别把固定管理员命令接到 `M10 → M00 admission/durable evidence → owning M71/M70 port`；但 B4b Serde values 仍只是 data-only，且这些 fixture-specific adapters 不等于 production evidence service、正式 SSO 或通用管理员 API。
