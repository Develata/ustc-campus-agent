...
| 校园任务 | 当前结果 | 数据与使用条件 |
|---|---|---|
| 办理成绩单证明 | 查询条件、步骤、官方入口；导出个人办理清单 | 固定办理流程；可扩展检索与获取管理员配置的官方来源 |
| 查看校历变化 | 查看版本差异、来源和变更板 | 固定校历样例；资料工作区另保留真实观察和版本差异 |
| 比较课程方案 | 按个人课程资料、兴趣、先修和空闲时间比较方案 | 仅本次明确同意；引用输入原文，不采集 iCourse 评分 |
| 管理个人事项 | 日期新增／修改／删除与批量课程先确认，重启后保留 | 北京时间明确显示，到点投递站内提醒并保留回执 |
| 给 Agent 添加能力 | 安装、配置、授权、启用、更新／回滚和撤销 | 已审阅的只读 MCP、Skill 或混合包；导入先审阅文件 |

对话支持历史保存、继续追问、置顶、分组和删除；按日期排列，重命名保留 `YYMMDD|` 前缀。
设置中可保存个人 Agent 根提示词，为后续对话设定角色、任务处理方式和回答习惯。
模型选择紧邻发送键；回答增量显示，执行可停止，重启后保留已完成工具进度。
停止不会回滚已经执行的操作。管理员控件独立收纳。
...
选课可在插件页输入实际课程原文、偏好和空闲时间；旧演示档案入口仍保留在演示模式。
多人登录、来源清单和课程到日历流程见 [校园任务指南](docs/guides/campus-workflows.md)。
...
- 办事与校历保留固定审阅样例；新增受管理员来源清单约束的 HTTPS 获取、文本导入与修订检索，默认无已授权的联网来源。已有 iCourse 聚合评分快照的数据使用许可仍待闭环。
- 课程资料、兴趣与空闲时间按本次请求同意。批量日历须确认完整提案；提醒仅投递站内收件箱，已有投递回执。Agent 支持流式文本、停止执行和持久进度，不自动重放中断写入。
- 服务默认只监听 loopback；密钥从私有服务端文件读取，不进入页面、仓库或工具回执。
- 后台配置用户已支持登录／退出、逐请求认证与私有日历、会话和插件状态隔离，无自助注册。当前仅验证本地 loopback；公网部署和学校 SSO 仍需单独配置与验收。
