...
| `M00 Platform Control/Identity` | `platform-account.md`, `platform-account-local.md`, `platform-identity.md`, `platform-session.md`, `platform-session-port.md`, `platform-control-evidence.md`, `platform-request-context.md`, `B-M00-M10-ACTOR` | bounded B4a session read, B4b redacted evidence ports and fixed B5 Affairs/ChangeRadar administrator compositions are implemented; each fixed command persists or verifies admitted-request evidence before its owning product effect, and bounded operator-configured loopback accounts admit owner-scoped requests; production evidence service/SSO remain planned | `active:AUTH-011`; `active:AUTH-012`; `active:AUTH-013`; `active:AUTH-014`; `active:AUTH-015`; `active:AUTH-016`; `active:AUTH-017`; `active:AUTH-018`; `active:AUTH-019`; `active:AUTH-020`; `active:AUTH-021`; `active:AUTH-022`; `active:AUTH-023`; `active:AUTH-024` |
| `M10 Application Ingress Host` | `market-catalog-query.md`, `agent-chat.md`, `B-M80-M10-CALL`, `B-M10-M80-RESULT`, `B-M10-M80-EVENT`, `B-M10-APP-CALL`, `B-APP-M10-RESULT`; canonical application-operation/schema and per-adapter allowlist registry in `interfaces.md` | query carriers, fixed typed `affairs.publish`/`change.publish` commands, four static M72 private operations, the closed loopback `agent-chat/v1` route, safe provider-status query and explicit tool-free local-chat profile and loopback HTTP/CLI/Web projections; configured-account Web admission and bounded stream/stop routes have local composition implementations; Dioxus, public-read-first inbound MCP and public production admission remain later journeys | `active:CHAT-*`; `active:AUTH-023`; `active:AUTH-024`; `active:PROC-011`; `active:RADAR-001`; `active:CLIENT-007`; `active:CLIENT-009`; `active:CLIENT-010`; `long-horizon:CLIENT-001`; `long-horizon:CLIENT-002`; `long-horizon:CLIENT-003`; `long-horizon:CLIENT-004`; `long-horizon:CLIENT-005`; `long-horizon:CLIENT-006`; `long-horizon:WEB-*` |
| `M20 Market/Package` | `plugin-package.md`, `market-lifecycle.md`, `market-component-configuration.md`, `invocation-resolution.md`, package/install/grant/resolver contracts, `B-M20-M72-AUTH` | `00-market-browse-install`; pure package-owned `M20-CONFIG-001/002` schema validation and exact component binding support configuration; bundled catalog query/browser has bounded `MARKET-008` evidence, while `plugin-management.md` now binds durable configuration, mixed packages, inert import review and disabled-only exact B6 update/rollback to PLUGIN-001; current bounded M72 composition consumes transaction-current static application authorization without creating a tool projection | `active:MARKET-*`; `active:PKG-*`; `active:AGENT-*`; `active:FP-*`; `active:PLUGIN-001` |
| `M30 Agent Harness/Runtime` | `agent-harness.md`, `agent-runtime.md`, `agent-chat.md`, `chat-conversations.md`, `chat-activity.md`, `B-M30-M50-MODEL`, `B-M20-M30-TOOLSET`, `B-M30-M40-CALL`, `B-COMP-M30-EFFECT`, `B-M40-M30-RESULT` | `04-bounded-agent-harness`; a bounded app-private three-turn/four-call Chat coordinator owns M30 provider/tool orchestration for Affairs/ChangeRadar and may invoke only the separately authorized static M72 plan use case; M72 consent/profile/planning semantics are not M30/M40 evidence, bounded conversation SSE, stop and completed-tool checkpoints are implemented under CHAT-004/005, while the durable full harness remains planned | `active:CHAT-001`; `active:CHAT-002`; `active:CHAT-004`; `active:CHAT-005`; `active:HARNESS-*`; `active:AGENT-*` |
...
| `M50 Model Provider` | `agent-chat.md`, `B-M30-M50-MODEL` | deterministic no-network mock and one bounded operator-configured OpenAI-compatible adapter with real SSE and complete JSON fallback are implemented for the Chat MVP; the complete provider platform remains a gap | `active:CHAT-001`; `gap`; `long-horizon:AI-*` |
| `M51 MCP Binding/Executor` | `B-M40-M51-EXEC`, `mcp-execution.md`, `plugin-management.md` | bounded Streamable HTTP adapter, local schema references and single/mixed-component public-read package journeys; broader MCP baseline remains later | `active:MCP-019`; `active:PLUGIN-001`; `long-horizon:MCP-001`; `long-horizon:MCP-002`; `long-horizon:MCP-003`; `long-horizon:MCP-004`; `long-horizon:MCP-005`; `long-horizon:MCP-006`; `long-horizon:MCP-007`; `long-horizon:MCP-008`; `long-horizon:MCP-009`; `long-horizon:MCP-010`; `long-horizon:MCP-011`; `long-horizon:MCP-012`; `long-horizon:MCP-013`; `long-horizon:MCP-014`; `long-horizon:MCP-015`; `long-horizon:MCP-016`; `long-horizon:MCP-017`; `long-horizon:MCP-018` |
| `M60 Campus Trust/Source` | `source-import.md`, `source-retrieval.md`, `campus-source-workspace.md`, `B-M60-M70/71/72-*`, `B-M60-M90-SOURCE-TRANSPORT` | all first-party features; bounded local source observations/search are separate from canonical publication and original B3 retrieval acceptance | `active:SRC-*`; `active:PROC-*`; `active:RADAR-*`; `active:COURSE-*`; `active:FP-*` |
...
[Platform accounts](contracts/platform-account.md) owns account construction under M00.
[The local adapter](contracts/platform-account-local.md) implements operator-configured
loopback login/logout and current-session owner admission; live SSO remains planned. [The multi-user delivery task](tasks/multi-user-campus-agent.md)
...
reload/switch/continuation and explicit interrupted recovery. The configured-account composition now supplies the admitted owner. STREAM-CANCEL-001
adds true provider SSE, explicit stop and version-3 completed-tool checkpoints; restart
never redispatches interrupted tools. Full Harness and public deployment remain planned.
Account entry remains administrator-provisioned users or separately gated SSO.
...
STREAM-CANCEL-001 adds incremental answer text, exact-request stop and safe recovery
projections. Activity remains rebuildable; product stores and receipts own effects,
and a stopped response never claims that a committed effect was cancelled.
...
the Agent. The profile now admits reviewed mixed components, inert import previews and disabled-only
exact-version B6 update/rollback with fresh probe/grants after pin changes. It does not
promote production SSO, private/write approvals, stdio, executable Skills or full module exits.
...
CALENDAR-WORKSPACE-001 adds authenticated owner workspaces, atomic 1–32-item batch
confirmation and durable station-inbox reminder delivery/read receipts. Legacy demo
records retain their original ownership. External push/email/SMS and production
operations remain outside this profile; no module readiness or existing acceptance-row
status is promoted by this extension.

## Local source and course extension

[Local source observations and personal planning](contracts/campus-source-workspace.md)
projects through [campus workflows](guides/campus-workflows.md) and
[the feature](features/campus-source-workspace.md). S1-LOCAL-OBS-001 covers exact
operator-reviewed public USTC URLs, bounded HTTPS/import, immutable observations,
search and line differences. O1-PERSONAL-PLAN-001 covers request-only consent,
supplied excerpts, deterministic course constraints and Calendar batch suggestions
with separate human confirmation. These case IDs are bounded contract-level proof
obligations, not new active matrix passes. Production retrieval/B3, canonical
publication, real source permission, verified curriculum facts and iCourse ingestion
are not inferred from local tests. Final multi-user/browser and live-source evidence
is tracked by the delivery task; existing planned rows retain their status.
