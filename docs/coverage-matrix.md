...
| `M90 Infrastructure/Operations` | `agent-chat.md`, module-owned ports plus Docker Compose Fullstack deployment/recovery contracts | loopback deterministic-mock Compose MVP package with operator-created password-verifier secret, stop/restart product persistence, process-local access-session invalidation and reset-only volume deletion; no independent product behavior | `active:CHAT-003`; `active:RUNTIME-*`; `active:PUBLIC-*`; `long-horizon:CFG-*`; `long-horizon:REL-*`; `long-horizon:DEP-*` |
