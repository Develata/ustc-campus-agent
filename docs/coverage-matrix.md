...
| `M30 Agent Harness/Runtime` | `agent-harness.md`, `agent-runtime.md`, `agent-chat.md`, `B-M30-M50-MODEL`, `B-M20-M30-TOOLSET`, `B-M30-M40-CALL`, `B-COMP-M30-EFFECT`, `B-M40-M30-RESULT` | `04-bounded-agent-harness`; a bounded in-memory chat coordinator is implemented for Affairs/ChangeRadar and may call only the pre-authorized static M72 plan operation, while the durable full harness remains planned | `active:CHAT-001`; `active:CHAT-002`; `active:HARNESS-*`; `active:AGENT-*` |
...
| `M80 Client Core and Interaction Shells` | `agent-chat.md`, `client-shell.md`, `cli.md`, `interfaces.md`, `permissions.md`, `B-M80-M10-CALL`, `B-M10-M80-RESULT`, `B-M10-M80-EVENT` | implemented thin static Web Chat plus framework-neutral client core; peer Dioxus Web/Android, `ustc-agent` and public-read-first inbound MCP remain later journeys; Windows launchers do not promote Windows GUI target support | `active:CHAT-001`; `active:CHAT-002`; `active:CLIENT-007`; `active:CLIENT-008`; `active:CLIENT-009`; `active:CLIENT-010`; `long-horizon:CLIENT-001`; `long-horizon:CLIENT-002`; `long-horizon:CLIENT-003`; `long-horizon:CLIENT-004`; `long-horizon:CLIENT-005`; `long-horizon:CLIENT-006`; `long-horizon:WEB-*` |
