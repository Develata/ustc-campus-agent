...
| `M00 Platform Control/Identity` | `platform-identity.md`, `platform-session.md`, `platform-session-port.md`, `platform-control-evidence.md`, `platform-request-context.md`, `B-M00-M10-ACTOR` | cross-feature admission only; bounded B4a session ports/durable current-session read vendor and B4b redacted control-evidence/journal ports are implemented, completing the typed B4 interface/fake batch while B5 M10 administration and production evidence persistence remain planned | `active:AUTH-011`; `active:AUTH-012`; `active:AUTH-013`; `active:AUTH-014`; `active:AUTH-015`; `active:AUTH-016`; `active:AUTH-017`; `active:AUTH-018`; `active:AUTH-019`; `active:AUTH-020`; `active:AUTH-021`; `active:AUTH-022` |
| `M10 Application Ingress Host` | `B-M80-M10-CALL`, `B-M10-M80-RESULT`, `B-M10-M80-EVENT`, `B-M10-APP-CALL`, `B-APP-M10-RESULT`; canonical application-operation/schema and per-adapter allowlist registry in `interfaces.md` | Dioxus, `ustc-agent`, public-read-first inbound MCP and other admitted integration journeys | `active:CLIENT-007`; `active:CLIENT-009`; `active:CLIENT-010`; `long-horizon:CLIENT-001`; `long-horizon:CLIENT-002`; `long-horizon:CLIENT-003`; `long-horizon:CLIENT-004`; `long-horizon:CLIENT-005`; `long-horizon:CLIENT-006`; `long-horizon:WEB-*` |
