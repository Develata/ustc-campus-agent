...
For the same reason the compiled module graph is pinned rather than the file extension. Which files Cargo compiles into `platform-core` is decided by its non-inline `mod` declarations, so those are frozen exactly: `lib.rs` declares `identity`, `invocation`, `market` and `session` and nothing else; `market` declares `capability` and `installation`; and none of the others declares a submodule. That inventory is registered surface, so it moves whenever an admitted module is added and each move is mirrored in both carriers. Pinning the declarations pins the compiled set semantically, so no attribute spelling can introduce a module the evidence never reads — not `#[path]`, not `#[cfg_attr(all(), path = "hidden.txt")]`, and not a future one, whatever file extension it targets. Enumerating `*.rs` files cannot do this on its own, because a module's source need not end in `.rs`.
...
All five identity rows are `implemented`: the named Rust tests, the rustdoc `compile_fail` API proofs and the implementation-specific `check_platform_identity_implementation` checker rules exist and pass. `AUTH-013` is now separately implemented by [`platform-request-context/v0`](platform-request-context.md); it consumes the same exact six identity kinds and owns `CausationId` locally, without widening this contract's nominal kind set.
