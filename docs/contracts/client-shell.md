...
`crates/client-core` and the real `apps/ustc-agent` binary retain one typed `affairs get/lookup` path over numeric loopback framing. `ustc-agentd serve-web` additionally retains one loopback-only, operation-specific Axum/HTTP presentation proof whose static chat-first shell renders server-owned Chat/tool/product projections without making domain decisions. It presents the M10-owned deployment-local login/session state, current page-lifetime conversation and secondary tools view, but offers no registration, durable conversation list or production auth claim. The CLI core validates endpoint/protocol/value bounds, propagates correlation/idempotency/provenance, reduces typed M10 responses/errors, emits one canonical JSON result and maps stable exit classes without importing backend or operator implementations. The colocated Web proof is not a replacement for that core and does not count as the future peer Dioxus shell. This is partial evidence only: no production profile/auth, remote HTTP/TLS, NDJSON/SSE, reconnect/cancellation/version-skew matrix, inbound MCP or Dioxus target exists.
