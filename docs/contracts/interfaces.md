...
| `agent.chat.turn` | M30 | `public-read` plus per-request tenant-private consent when course advice is used | bounded provider/tool execution | loopback Web/HTTP | accepted bounded chat + first-party Plugin MVP contract; implementation evidence pending |
...
All four operations use one bounded `SubmitOpportunityDto` envelope and a closed typed command sum. The envelope carries request/correlation/causation/idempotency identities, an **authenticated** session reference, client provenance, one mandatory per-request `confirmed | not_confirmed` user-confirmation intent and a domain-separated payload digest over both the command and that confirmation intent. An absent or unknown DTO field is malformed and reaches no M00/M20/application dispatch. The envelope carries no tenant or user identifier; M10 forwards only the `M00AdmittedActor::Authenticated` identities.
...
| `/api/v1/agent/chat` | POST | `agent.chat.turn`; one bounded non-streaming provider turn with at most one strict Affairs or course-advice Plugin call and one final provider turn | accepted loopback MVP contract; protocol major `1`; production compatibility not claimed |
...
The bounded Web demo also serves `/`, `/assets/styles.css`, `/assets/app.js` and `/healthz` from the same `ustc-agentd serve-web` process. The Affairs form accepts only a procedure ID; the ChangeRadar section requests one fixed reviewed board identity and offers its sibling Atom projection. The active Opportunity Graph section uses only the server-owned demo session and synthetic profile input; the session/tenant/user identifiers are never accepted from a browser field or returned to JavaScript. Every Opportunity route also requires `X-USTC-Opportunity-Confirmation: confirmed`; the adapter maps missing or different values to `not_confirmed`, and transaction-current M20 authority denies before static dispatch. The header is explicit caller intent, not authority by itself. All sections render only server-owned typed results with `textContent`; the page performs no source, freshness, conflict, eligibility, procedure, semantic-diff or planning calculation. The server creates bounded request/correlation identities, recomputes each operation's command-plus-confirmation payload digest and invokes the ordinary M00/M10 admission path. For Affairs, the response-only public capability minted by submit is consumed immediately by an internal typed lookup and MUST NOT be serialized, logged, stored in browser state or placed in a URL. ChangeRadar returns its bounded synchronous typed terminal. Opportunity responses are owner-private and are never written to the public M10 lookup store. Static and API responses are `no-store`, `nosniff`, frame-denied and same-origin constrained. This demo surface is not a compatibility promise for the future Dioxus/production API.

The bounded chat route accepts only `message` and `course_profile_consent`.
The message is trimmed and bounded to 8192 UTF-8 bytes. The server-selected
provider profile comes only from process environment and is never accepted from
the request. `course_profile_consent` permits only the one request's
tenant-private M72 create/generate/revoke-delete sequence; it is neither Market
authority nor enrollment consent. The model can request one of the two frozen
tool schemas, but the host validates arguments and invokes existing typed
application/composition paths. A successful course-advice tool call is not
returned until revoke-delete succeeds. The assistant answer names used tools and
whether a typed Plugin result grounded the answer, but the answer itself is not
source or enrollment authority. Exact wire and provider configuration are owned
by [`chat-plugin-mvp.md`](../tasks/chat-plugin-mvp.md).
