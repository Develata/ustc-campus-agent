...
| `B-M30-M50-MODEL` | `M30` | `M50` | complete model request, ordered events, usage and typed provider errors | bounded Chat MVP subset implements deterministic mock plus one non-streaming OpenAI-compatible request/final/error path; ordered streaming, normalized real usage and full provider conformance remain planned |
