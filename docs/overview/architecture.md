...
| `M90` | Infrastructure/Operations | ports for storage/journal/evidence/config/secrets/HTTP/telemetry and Docker Compose deployment | CI/checker plus app-private secure canonical-JSON stores for bounded session/evidence/Affairs publication/ChangeRadar publication/Opportunity fixture state; no production database claim |
