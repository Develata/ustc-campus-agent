...
标量约束支持整数／数字的 `minimum`、`maximum`，以及字符串的 `minLength`、
`maxLength`；范围包含端点，长度按 Unicode 字符数计算，仍受独立 UTF-8 字节上限限制。
整数阈值必须是 i64 范围的整数写法，长度必须是非负 u64 整数写法；数字阈值使用有限
binary64，并限定在 [-2^53, 2^53]；超出范围或不能精确表示的整数阈值拒绝，其他小数沿用该数值精度。约束会实际校验并参与
审核摘要，修改后需重新检查。独占范围、正则和 `multipleOf` 暂不支持。
...
插件页的“准备 MCP / Skill 导入包”可把 Skill 原文与 MCP 工具到公开读取能力的
明确映射转换为候选包。先检查完整文件和来源，再下载审阅包 JSON；页面不会自动安装、
访问候选地址或执行代码。审阅后由管理员将文件放入包目录并接入既有目录配置，用户再
完成安装、配置、检查、授权和启用。候选清单中的 tier 是待审阅分类，并不代表已经审阅。

一个包可包含 Skill 与一个 MCP 服务，统一安装、配置、启用和停用。组合包使用
`plugin-runtime/v2` 的组件列表，每项有 `componentId` 和现有 v1 `runtime`；所有组件
绑定同一个包级配置 schema。任一组件检查失败都不会得到可启用的完整检查结果。
精确格式与边界见 [管理契约](../contracts/plugin-management.md)。

管理员接入同一包的其他已审阅版本后，在已安装包的“版本更新与回滚”中先停用、检查
目标版本，再核对能力与来源分类并确认更新。新版本保持停用，旧授权全部失效，需要
重新检查、逐项授权和启用。保留的旧版本可先检查再确认回滚；回滚也不恢复旧授权。
确认保留当前版本会结束本次回滚窗口。当前配置必须同时适用于两个版本，否则检查
会拒绝，需要管理员先完成受审阅的配置迁移方案。目录缺失或文件变化会阻断切换。

更新记录和安装/授权一起保存，重启可恢复同编号请求结果。stdio、执行 Skill 脚本
及私有/写能力仍不在当前范围。v2 状态文件不能交给只识别 v1 的旧服务程序直接读取。
