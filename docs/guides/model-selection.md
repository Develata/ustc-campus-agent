...
在“设置 → Agent 根提示词”填写长期指令，例如：

> 你是我的校园学习助手。先给可执行步骤，再解释原因；选课建议区分课程事实和个人判断。

点击“保存”后，从下一条消息起用于已有和新建对话，刷新与服务重启后保留。
“恢复默认”清空已保存指令。“对话选项 → 响应偏好”仍只作用于单次请求。
字数计数按 UTF-8 字节计算，上限 8192 字节；支持多行。工具权限和操作确认规则仍然适用。
离线 mock 会保存设置，但不会模拟模型遵循指令的语言效果。
若提示版本冲突，先重新读取服务端设置再决定修改，避免覆盖另一页面刚保存的内容。
