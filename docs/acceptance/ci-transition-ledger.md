...
- Pull-request supersession cancellation is active; push-to-main runs use unique run-ID groups and are never cancelled by this policy.
...
| `CI-TR-002` | `pull_request` plus push-to-main trigger semantics remain | `.github/workflows/ci.yml` preserves both triggers, gives push runs unique groups, and cancels only superseded pull-request runs | `scripts/tests/fixtures/ci-v2.yml` preserves the trigger baseline without owning active cancellation | checker mutation tests remove each trigger, widen cancellation to push, or drift the concurrency group | `active-full-sharded` |
...
| `CI-TR-004` | full Python discovery may be replaced only by an exact full-suite proof with no omission | `.github/workflows/ci.yml` runs `python3 scripts/run_checker_shards.py --jobs 4 --timeout-seconds 1800` over the exact inventory; sorted IDs are deterministically round-robin striped so a large lexicographically adjacent `TestCase` class cannot monopolize one process | `scripts/tests/fixtures/ci-v2.yml` retains the same runner shape | bidirectional inventory coverage, exact-union fan-in, process isolation, deterministic striped partition tests, per-shard reports/timing, and missing/unexpected/duplicate/zero-ID mutation tests | `active-full-sharded` |
