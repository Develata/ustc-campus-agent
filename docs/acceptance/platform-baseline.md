...
| `AUTH-013` | bounded `platform-request-context/v0` admits a closed Public/Authenticated actor sum with no synthetic public identity, exact authenticated session binding, immutable request-scoped operation projection, exact closed permission/effect surfaces and fail-closed coherence, current policy/capability checks, fenced idempotency, complete scalar dispositions, fourteen payload-preserving rejection classes and validating persistence promotion across the exact 65-test inventory | rust-integration + rust-doc-test + contract-check | PR |
...
| `PROC-011` | first Affairs Navigator product slice binds administrator-imported reviewed snapshot → typed `ProcedureDraft` validated against board policy → administrator review and publish → one application query (exact stable ID or structured search) → thin Web result with conditions, steps, effective/deadline time, entry point, safe public lineage (`source_id`, `evidence_set_digest`, `materialization_receipt_id`, `revision_count`), source evidence, freshness, conflicts and uncertainty without a public capability or raw revision identity; explicitly does NOT close Agent runtime, Market artifact switching, Android, CLI, inbound MCP, SSE streaming or full client-protocol/version-skew coverage, which remain separate acceptance rows | rust-integration + browser-automation | demo |
...
| `CLIENT-007` | M10 owns one framework-neutral versioned operation/client-protocol carrier and never depends on M80 client-core; Dioxus, `ustc-agent` and inbound MCP consume one M80 framework-neutral typed client core over that carrier; per-adapter allowlists project one operation registry, and equivalent fake-M10 request/result/error/event, reconnect, cancellation and compatibility cases reduce to equivalent semantic client state | rust-integration | PR |
...
| `CLIENT-009` | real cross-platform `ustc-agent` read and stream paths preserve versioned JSON/NDJSON stdout, redacted stderr, stable exit classes, least-privilege auth, compatibility, cursor reconnect, cancellation distinction and timeout reconciliation; this active row covers the required initial host matrix and does not require a Windows binary or imply Windows GUI support | rust-cli-real-host | integration |
| `CLIENT-010` | inbound MCP Streamable HTTP discovery/invocation maps only reviewed bounded operation/schema projections through client-core and M10, begins public-read, binds delegated caller/session/tenant and applicable current grant state, invalidates widened schemas/grants, instruction-isolates results and reaches no operator, domain or outbound-M51 authority | external-conformance | integration |
