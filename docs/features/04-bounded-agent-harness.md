...
- `Contracts`: [`../contracts/agent-harness.md`](../contracts/agent-harness.md), [`../contracts/agent-plugin-boundary.md`](../contracts/agent-plugin-boundary.md), [`../contracts/agent-chat.md`](../contracts/agent-chat.md)
- `Acceptance`: `HARNESS-*`, `AGENT-017/018`, `PKG-019/020`, active `CHAT-*`
...
[`agent-chat/v1`](../contracts/agent-chat.md) governs a narrower retained integration profile before the full durable `HarnessRun`; [`02-agent-chat-mvp.md`](../tasks/02-agent-chat-mvp.md) only schedules its delivery. One in-memory finite Chat request uses either a no-network deterministic fake or one explicitly configured OpenAI-compatible adapter, permits at most three provider turns and four sequential tool calls, and returns a natural-language answer plus a safe tool trace. The loopback browser presents this through a chat-first shell behind one deployment-local login; that process-local access session is not production identity or M00 authority.
