...
Android 是连接同一服务的 WebView 壳。本次同源 APK 构建、4 项端点测试、lint 和签名
检查通过；机主解除安装限制后，小米 API 35 真机安装、Chat 首页和 WebView 日历查询通过。
[检查记录](../guides/android-demo.md#当前验证结果)没有覆盖完整模型回合、
键盘、返回键、旋转、进程重建与离线恢复，不提升长期客户端验收状态。
