...
`ustc-agentd serve-web` separately provides one operation-specific loopback presentation proof over the same M10-owned typed DTO. Its chat-first shell renders page-lifetime conversation, authoritative tool traces, a secondary campus-tools view and the M10-owned local-access login/session state; it offers no registration or durable conversation list. It consumes the public capability internally and exposes no raw revision identity. Because this proof is colocated in the composition root and does not use the shared client core or Dioxus, it does not establish the peer Web/PWA target or production authentication adapter.
