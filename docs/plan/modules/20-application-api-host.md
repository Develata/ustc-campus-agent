...
The retained `client-protocol` and `application-ingress` carriers prove checked request/value/error envelopes, M00 admission, capability and owner/operator lookup projections, idempotent submit/finalize behavior and typed M71 result mapping. They also own fixed typed Affairs and ChangeRadar administrator commands whose payload digests are recomputed before M00 admission and whose admitted-request evidence is durable before the owning product port is called. `apps/ustc-agentd` composes the fixture-backed three-plugin surface over numeric loopback TCP, including owner-only canonical Affairs and ChangeRadar publication state, operator CLI/HTTP/Web calls and public JSON/Atom read-back after restart. The same loopback composition owns one bounded local-access login/session adapter for protecting provider-backed Chat and tenant-private Opportunity routes. These are bounded demo surfaces, not production remote administration, production M00 identity, SSO, multi-user tenancy or a generic operation registry.

### Bounded loopback local-access gate

The `agent-chat/v1` Compose profile admits one deployment-local username and one Argon2id PHC password verifier supplied by an M90 secret-file reference. M10 verifies login requests, mints an unpredictable opaque browser session, enforces idle and absolute expiry, and rejects unauthenticated Chat and private Opportunity requests before provider or product I/O. The session is process-local and deliberately invalidated by daemon restart or password rotation. Same-authority Origin admission, JSON-only login, `HttpOnly; SameSite=Strict` cookies and bounded failed-login throttling form the demo CSRF/brute-force boundary; loopback HTTP cannot claim the production `Secure`-cookie/TLS boundary.

Static assets, health/bootstrap compatibility and already-public Affairs/ChangeRadar reads remain public. Existing fixed administrator publication endpoints retain their separate loopback operator-CLI confirmation contract and do not derive M00 administrator authority from the local-access cookie. The browser labels the account as local deployment access, offers no registration, and cannot turn it into USTC/CAS identity or domain permission.
