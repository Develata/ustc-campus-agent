...
|---|---|---|---|---|---|
| `M00` | Platform Control and Identity | `partial-evidence` | tenant/user/session identity, request causation, application command envelope, platform-wide policy references | package internals, Agent loop, source parsing, UI | identity-types, session-domain, bounded [`platform-request-context/v0`](../../contracts/platform-request-context.md) / `AUTH-013`, B4a [`platform-session-port/v0`](../../contracts/platform-session-port.md) / `AUTH-021`, and B4b [`platform-control-evidence/v0`](../../contracts/platform-control-evidence.md) / `AUTH-022` are implemented; the typed B4 interface/fake batch is complete, while B5 administration, production evidence persistence and formal SSO remain planned |
| `M10` | Application Ingress Host | `partial-evidence` | Dioxus/Axum server-function ingress, versioned public HTTP/typed streams where needed, auth/session/client-compatibility admission, mapping to application ports | domain decisions, direct database/executor rules, UI | framework-neutral client-protocol/application-ingress carriers plus bounded loopback Affairs and ChangeRadar compositions exist; M10-owned actor-aware invocation ports route admitted calls through current Market/Harness/ToolGateway paths with no direct production fallback; public remote HTTP/Dioxus hosting, durable ChangeRadar terminal records and the broad operation registry remain planned |
...
| `M70` | USTC ChangeRadar | `partial-evidence` | semantic change candidates/events, board scope, approval and feed behavior | generic source authority, other product state | exact-source-pinned semantic diff + digest-bound administrator approve/reject + coherent M60 verification + exactly-once in-memory publication + deterministic Atom now compose through fixture-backed `Web → M10 change.list → current Market authorization → bounded Harness/ToolGateway → fixed ChangeRadar adapter → owning query service → M10/Web`; disable/revoke and Affairs-isolation paths fail closed; no M00-authorized administration, durable adapter/restart, M80 peer-client composition or complete product journey |
...
- `crates/change-radar` plus the fixed `apps/ustc-agentd` adapter/fixture/Web projection are partial `M70` evidence for bounded board policy, typed semantic diff, deterministic candidate identity, administrator review receipts, atomic in-memory publication, current Market/Harness/ToolGateway invocation and deterministic JSON/Atom projection; they are not M00-authorized administration, durable restart, real source retrieval or M80 peer-client evidence.
