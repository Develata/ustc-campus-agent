...
| `M50` | Model Provider Integration | `planned` | typed provider profiles, request/stream normalization, token estimation, timeout/cancel/error mapping | run authority, tool grants, prompt truth | bounded server-environment OpenAI-compatible Responses contract accepted for the chat + first-party Plugin MVP; retained implementation evidence remains pending and durable profiles/streaming/full safety remain planned |
