...
| `M40` | Tool Gateway and Execution | `partial-evidence` | Agent tool protocol mapping, call correlation, current authorization ordering, intent/executor/receipt/result sequence | grants, Agent phases, package declarations | protocol/fake conformance plus one bounded fixed first-party Affairs adapter composition implemented; durable generic gateway and out-of-process executor host planned |
...
| `M70` | USTC ChangeRadar | `partial-evidence` | semantic change candidates/events, board scope, approval and feed behavior | generic source authority, other product state | exact-source-pinned semantic diff + digest-bound administrator approve/reject + coherent M60 publication verification + exactly-once in-memory publication + deterministic Atom in `crates/change-radar`; no M00-authorized administration, durable adapter/restart, M10/M20/M40/M80/Web composition or complete product journey |
...
- `crates/change-radar` is partial `M70` evidence for bounded board policy, typed semantic diff, deterministic candidate identity, administrator review receipts, atomic in-memory publication and deterministic Atom; it is not M00-authorized administration, durable restart or platform/Web composition evidence.
