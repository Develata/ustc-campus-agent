...
|---|---|---|---|---|---|
| `M00` | Platform Control and Identity | `partial-evidence` | tenant/user/session identity, request causation, application command envelope, platform-wide policy references | package internals, Agent loop, source parsing, UI | identity-types and session-domain plus bounded [`platform-request-context/v0`](../../contracts/platform-request-context.md) / `AUTH-013` kernel implemented; production B4 ports/vendors and B5 composition planned |
| `M10` | Application Ingress Host | `partial-evidence` | Dioxus/Axum server-function ingress, versioned public HTTP/typed streams where needed, auth/session/client-compatibility admission, mapping to application ports | domain decisions, direct database/executor rules, UI | framework-neutral client-protocol/application-ingress carriers and one bounded loopback Affairs composition exist; public HTTP/Dioxus hosting, streaming/version matrix, graceful lifecycle and broad operation registry remain planned |
...
| `M70` | USTC ChangeRadar | `partial-evidence` | semantic change candidates/events, board scope, approval and feed behavior | generic source authority, other product state | exact-source-pinned board policy + deterministic typed semantic diff + complete-policy-bound candidate ID + service-minted atomic commits to an explicitly bounded in-memory baseline/candidate repository in `crates/change-radar`; no administrator review/publication, feed, durable adapter, M10/M20/M40/M60/M80 composition or product journey |
| `M71` | USTC Affairs Navigator | `partial-evidence` | reviewed tree/procedure artifacts, lookup, supersession and publication journey | generic source authority, full-corpus RAG as truth | checked query/freshness/conflict kernel plus exact M60 `DemoReviewed` draft import, one coherent M60-owned source-health/retained-evidence publication decision, digest-bound administrator approval and CAS/idempotent atomic publication into an explicitly bounded in-memory repository; `ustc-agentd` now bootstraps that publication and serves the same repository through fixture-backed M00→M10→M71 CLI/loopback-Web queries; committed tombstones replay after later revision/revocation while uncommitted retries recheck M60; no M00-authorized operator publication, durable adapter/restart, real source approval, structured search, supersession or complete Web journey |
...
- `crates/change-radar` is partial `M70` evidence for bounded board policy, typed semantic diff, deterministic candidate identity and atomic in-memory baseline advancement; it is not administrator/feed/composition evidence.
