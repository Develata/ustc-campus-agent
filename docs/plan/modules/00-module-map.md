...
| `M30` | Agent Harness and Runtime | `partial-evidence` | finite user-task/run phases, graph, budgets, context projection, provider/tool ports, replay and review | package lifecycle, executor implementation, transport/UI | node runtime kernel, provider-free deterministic Harness turns/replay and one bounded app-private three-turn/three-call Chat coordinator over M50 plus exact sequential tools implemented; durable finite user-task harness/graph remains planned |
