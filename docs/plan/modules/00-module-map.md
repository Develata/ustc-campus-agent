...
| `M71` | USTC Affairs Navigator | `partial-evidence` | reviewed tree/procedure artifacts, lookup, supersession and publication journey | generic source authority, full-corpus RAG as truth | checked query/freshness/conflict kernel plus exact M60 `DemoReviewed` draft import, one coherent M60-owned source-health/retained-evidence publication decision, digest-bound administrator approval and CAS/idempotent atomic publication into an explicitly bounded in-memory repository; committed tombstones replay after later revision/revocation while uncommitted retries recheck M60; existing loopback query/Web still uses a separate fixture composition; no M00 administrator auth, durable adapter/restart, real source approval, publication application composition, structured search, supersession or complete Web journey |
