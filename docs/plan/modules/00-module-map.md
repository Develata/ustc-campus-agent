...
|---|---|---|---|---|---|
| `M00` | Platform Control and Identity | `partial-evidence` | tenant/user/session identity, request causation, application command envelope, platform-wide policy references | package internals, Agent loop, source parsing, UI | identity-types, session-domain, bounded [`platform-request-context/v0`](../../contracts/platform-request-context.md) / `AUTH-013` and bounded [`platform-session-port/v0`](../../contracts/platform-session-port.md) / `AUTH-021` are implemented; B4a includes one secure durable DemoReviewed current-session read/bootstrap vendor, while B4b control-evidence, B5 administration and formal SSO remain planned |
