...
- `Version`: `module-map/v3.2`
- `Last Review`: `2026-09-03`
- `Owning Constitution`: [`../00-engineering-constitution.md`](../00-engineering-constitution.md)
- `Counterpart Contracts`: [`../../contracts/module-boundaries.md`](../../contracts/module-boundaries.md), [`../../contracts/agent-chat.md`](../../contracts/agent-chat.md)
...
| `M30` | Agent Harness and Runtime | `partial-evidence` | finite user-task/run phases, graph, budgets, context projection, provider/tool ports, replay and review | package lifecycle, executor implementation, transport/UI | node runtime kernel, provider-free deterministic Harness turns/replay and one bounded app-private three-turn/four-call Chat coordinator over M50 plus exact sequential tools implemented; durable finite user-task harness/graph remains planned |
...
| `M50` | Model Provider Integration | `partial-evidence` | typed provider profiles, request/stream normalization, token estimation, timeout/cancel/error mapping | run authority, tool grants, prompt truth | deterministic mock plus one bounded non-streaming operator-configured OpenAI-compatible Chat adapter with complete-request context preflight, typed errors and redacted file-backed secret handling; complete profile/stream platform planned |
...
| `M60` | Campus Trust and Source Pipeline | `planned` | source registry, retrieval policy, immutable revision, normalization, provenance, accepted baseline and publication gate | product-specific rendering, arbitrary crawling, UI | `source-import/v1` and `source-retrieval/v0` accepted under R11 per `ACCEPT_EXACT_M60_B2_R11_PACKET`; bounded B1 registry, first offline-only B2 pure policy, canonical-URL-bound deterministic-ID `DemoReviewed` revision values and an M60-owned health type exist; no transport/network effect, approved source, parser, durable baseline or publication composition; planner fixtures provide limited evidence; superseded V10 `DEC-M60-B2-ACCEPTANCE` is historical |
...
| `M72` | Campus Opportunity Graph | `partial-evidence` | opportunity facts, qualification/dependency/conflict, tenant profile projection and planning journeys | public source authority, cross-user profile state | offline Course Planning plus bounded `M00/M10 → transaction-current M20 authorization including typed Ask confirmation → static M72 application use case` composition for exact consent, tenant-private durable profile/tombstone state, qualification/planning, source/profile staleness and revoke/delete; no Agent/Harness/provider/ToolGateway/PluginExecutor spine, approved live M60 source, production SSO or durable production M20 adapter |
| `M80` | Client Core and Interaction Shells | `partial-evidence` | framework-neutral typed client behavior; peer Dioxus Web/Android, `ustc-agent` user/automation CLI and inbound MCP adapters; later admitted Windows peer, with iOS/other desktop candidates later | domain calculation/mutation, direct repository/executor access, Agent/Market/Plugin/source authority, operator `ustc-agentctl`, outbound M51 execution, peer-shell subprocess dependencies | bounded client-core, real `ustc-agent affairs get/lookup` loopback JSON/exit path and a source-bound pre-Dioxus Java WebView debug bridge exist; generic fake/real conformance, HTTP/TLS, streaming/reconnect/cancellation matrix, inbound MCP and Dioxus Web/Android remain planned |
| `M90` | Platform Infrastructure and Operations | `partial-evidence` | repository implementations for storage, journal, evidence, clock, queue, config, secrets, telemetry and Docker Compose deployment/recovery wiring | domain transition rules and product policy | CI/checkers, app-private canonical-JSON persistence and one bounded loopback Docker Compose MVP package with deterministic archives, least-privilege runtime, persistent-volume restart/read-back and Windows launcher evidence; production infrastructure planned |
