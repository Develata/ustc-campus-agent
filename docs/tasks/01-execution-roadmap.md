...
|---|---|---|---|---|---|
| `M00` Platform Control/Identity | `partial-evidence` | identity-types, session-domain, bounded [`platform-request-context/v0`](../contracts/platform-request-context.md) (`AUTH-013`) and bounded B4a [`platform-session-port/v0`](../contracts/platform-session-port.md) (`AUTH-021`) implemented; B4a includes one secure durable DemoReviewed current-session read/bootstrap vendor, while B4b control-evidence and B5 administration remain planned | B4b control-evidence, then one B5 administrator-admission composition path | unassigned | B4b redaction proof plus admitted/denied administrator request with no downstream effect on denial |
...
- `M00-B3 request-context`: bounded platform-core implementation complete under `platform-request-context/v0` / `AUTH-013`: closed Public/Authenticated actor sum, immutable request-scoped descriptor, exact closed permission/effect surfaces and fail-closed coherence, current policy/session/capability checks, fenced idempotency, complete scalar disposition, persistence projection, fourteen rejections and exact 65-test evidence. This does not complete B4 as a batch or B5 M10-v17 composition; bounded B4a session-port/read-vendor evidence is recorded separately below.
- `M00-B4 ports-and-fakes`: B4a `session-port` is bounded implemented under `platform-session-port/v0` / `AUTH-021`: exact repository/clock/secret-ref interfaces, deterministic fakes and one secure durable DemoReviewed current-session read/bootstrap vendor now replace fixture-session lookup in the existing composition. B4b `control-evidence` remains planned and must land as its own reviewable commit before B5.
