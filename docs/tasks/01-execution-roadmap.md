...
| `M71` Affairs Navigator | `partial-evidence` | hardened ProcedureDraft/artifact/policy/freshness/conflict kernel plus fixture-backed exact stable-ID composition; no real source/admin/Web lifecycle | complete the missing real reviewed-snapshot import, administrator review/publish receipt and thin-Web projection without widening to broad search/refresh | Hermes/default Integration Authority Owner; bounded OMO module lane | `PROC-010`/`PROC-011` remain planned until every temporal, administrator and Web assertion is bound |
...
| `M80` Client Core and Interaction Shells | `partial-evidence` | bounded client-core and real `ustc-agent affairs get/lookup` loopback JSON/exit path; no production auth/HTTP/stream, inbound MCP or Dioxus target | harden the least-privilege public-read CLI boundary, then add one narrow first-party Web adapter over the `PROC-011` server-owned typed result; generic conformance/Android/inbound MCP remain separate | Hermes/default Integration Authority Owner; bounded OMO module lane; Kimi K3 + Claude Opus 5 retain graphical UI/design ownership | active planned `PROC-011` thin-Web proof first; `CLIENT-*`, `WEB-*`, Android and deployment rows remain separate |
...
Current bounded evidence implements parts of B1/B2/B5 and a fixture-only B6 exact-query composition: stable IDs, typed draft/artifact/policy validation, freshness/conflict/uncertainty and one reviewed-snapshot fixture reach M10 and the ordinary-user client. B3 supersession, B4 real administrator review/publish/render, broad structured search and real M60/M80 Web attachment remain open.
...
| `M10` Application Ingress Host | framework-neutral client-protocol/application-ingress carriers + fixture-backed loopback Affairs composition (`partial-evidence`; `PROC-011` remains `planned`/non-pass) | one server-function/public HTTP query route + dispatch/error projection; SSE remains optional | full client-contract + version-skew | first Web adapter binding |
...
| `M71` Affairs Navigator | hardened ProcedureDraft/artifact/policy/freshness/conflict kernel + fixture-backed exact stable-ID query (`partial-evidence`) | real reviewed snapshot import + administrator review/publish + thin-Web typed result (`PROC-011`) | broad tree/search/targeted refresh | `M60` reviewed snapshot + `M10` query |
...
| `M80` Client Core/Shells | bounded client-core + public-read `ustc-agent affairs get/lookup` loopback adapter (`partial-evidence`; `PROC-011` remains `planned`/non-pass) | narrow first-party Web adapter over the frozen M10 result | full Dioxus Web/Android + generic CLI/inbound MCP conformance | `M10` operation registry freeze |
