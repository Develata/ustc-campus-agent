...
| `M71` Affairs Navigator | `partial-evidence` | exact M60 `DemoReviewed` draft import, coherent M60 publication decision, digest-bound administrator approval and CAS/idempotent atomic publication now compose with fixture-backed M00→M10→M71 CLI/loopback-Web queries over the same in-memory repository; committed tombstones replay after later revision/revocation while uncommitted retries recheck M60; no M00-authorized operator publication or durable adapter/restart | replace startup fixture authority with an M00-authorized administrator command and durable publication repository/restart read-back; broad search/refresh stays deferred | Hermes/default Integration Authority Owner; bounded OMO module lane | `PROC-010`/`PROC-011` remain planned until every temporal, administrator, restart and end-to-end assertion passes |
