...
| `M10` Application Ingress Host | `partial-evidence` | M10-owned client-protocol/application-ingress carriers plus one fixture-backed numeric-loopback Affairs composition; no public HTTP/Dioxus/stream host | replace the fixture transport with one admitted HTTP/server-function query while preserving the exact typed operation and denial semantics; inbound MCP, SSE and full compatibility hosting stay deferred | Hermes/default Integration Authority Owner; bounded OMO module lanes implement disjoint carriers | active `PROC-011` still requires administrator-import/publish plus black-box application-query/thin-Web proof; peer-client stream/version conformance remains separate |
...
| `M71` Affairs Navigator | `partial-evidence` | exact M60 `DemoReviewed` draft import, one coherent M60-owned source-health/retained-evidence publication decision, digest-bound administrator approval and CAS/idempotent atomic publication into an explicitly bounded in-memory repository now exist beside the earlier query/fixture/loopback evidence; committed tombstones replay after later revision/revocation while uncommitted retries recheck M60; no M00 administrator auth, durable adapter/restart or publication application/Web composition | bind M00-authorized administrator import/publish through M10, then query the same published repository through the real application/Web path; broad search/refresh stays deferred | Hermes/default Integration Authority Owner; bounded OMO module lane | `PROC-010`/`PROC-011` remain planned until every temporal, administrator, restart and end-to-end assertion passes |
...
| `M71` Affairs Navigator | M60 `DemoReviewed` draft import + one coherent source-health/retained-evidence decision + digest-bound review + bounded CAS/idempotent atomic publication, alongside exact-ID query/loopback evidence (`partial-evidence`) | M00-authorized publication and same-repository M10 query/Web composition + durable restart read-back (`PROC-011`) | broad tree/search/targeted refresh | `M60` reviewed snapshot + `M10` query |
