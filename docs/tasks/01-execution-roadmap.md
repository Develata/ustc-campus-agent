...
| `M70` ChangeRadar | `partial-evidence` | exact-source-pinned board policy, typed semantic diff, complete-policy-bound candidate identity and service-minted atomic update to an explicitly bounded in-memory baseline/candidate repository; no review/feed/platform composition | one reviewed semantic change + board/feed path | Hermes/default Integration Authority Owner; bounded OMO audit lane | `RADAR-*` current-scope rows |
...
| `M70` ChangeRadar | bounded board-policy + typed semantic-diff + atomic in-memory baseline/candidate evidence (`partial-evidence`) | durable DemoReviewed revision/baseline import + administrator review | board/feed + M10/M20/M30/M40/M80 composition | current three-plugin critical path |
