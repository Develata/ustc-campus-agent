...
| `M10` Application Ingress Host | `skeleton` | skeleton | one bounded server-function/application HTTP query route with typed DTO/error handling for active planned `PROC-011`; CLI, inbound MCP, SSE streaming and full compatibility hosting stay deferred | unassigned | active `PROC-011` black-box application-query/Web proof first; peer-client stream/version conformance remains separate |
...
| `M71` Affairs Navigator | `design-only` | design only | active planned `PROC-011`: one ProcedureDraft from a reviewed snapshot, board-policy validation, administrator review/publish, exact stable-id application query and thin-Web result; broad tree/search/refresh stays deferred | unassigned | `PROC-010`/`PROC-011` first-product proof |
...
| `M80` Client Core and Interaction Shells | `planned` | no code | one narrow first-party Web adapter over the `PROC-011` server-owned typed result; generic client-core extraction, Android, CLI, inbound MCP, SSE and version-skew coverage stay deferred | client/core/CLI/MCP unassigned; Kimi K3 + Claude Opus 5 lead Windows UI/design; GPT review/local optimization | active planned `PROC-011` thin-Web proof first; `CLIENT-*`, `WEB-*`, Android and deployment rows remain separate |
...
| `M10` Application Ingress Host | one server-function + bounded application HTTP query route (planned/admitted architecture; skeleton-only at present; active `PROC-011` acceptance row exists but remains `planned`/non-pass; SSE is not required) | dispatch-and-errors + event-stream | full client-contract + version-skew | first Web adapter binding |
...
| `M71` Affairs Navigator | — | one ProcedureDraft + board-policy validation + administrator review/publish + exact stable-id query + thin-Web typed result (`PROC-011`) | broad tree/search/targeted refresh | `M60` reviewed snapshot + `M10` query |
