...
| `M72` Opportunity Graph | `partial-evidence` | deterministic planning now composes exact consent, tenant-private durable profile/tombstone state, source/profile staleness, Market/Harness/ToolGateway authorization, M72 projection and loopback Web create/view/plan/revoke-delete; real restart read-back and private-payload erasure are proved | replace the DemoReviewed catalog and demo session with approved M60 import plus production M00/SSO authority; broader opportunity types remain deferred | Hermes/default Integration Authority Owner | bounded MVP composition/restart/deletion assertions pass; production source/identity rows remain planned |
...
Current bounded evidence covers the framework-neutral portions of B1/B2/B4/B7, fixed typed Affairs/ChangeRadar administrator commands and a fixture-backed loopback composition in `ustc-agentd`. It does not complete the operation registry, supported-version/upgrade matrix, B3 public server-function adapter, B5 stream, B6 lifecycle or B8 HTTP/SSE adapter; no batch is promoted beyond partial evidence by the loopback proof.
