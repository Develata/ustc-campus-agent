...
- `M40`: Agent tool protocol/fake conformance plus one bounded Affairs fixed-adapter composition proof;
...
| `M10` Application Ingress Host | `partial-evidence` | M10-owned client-protocol/application-ingress carriers plus fixture-backed Affairs TCP and loopback HTTP/Web compositions; admitted Affairs submits now pass through an M10-owned actor-aware invocation port before the bounded Market/Harness/ToolGateway/M71 path, with no direct production fallback | replace fixture authority with M00-authorized operator publication and a production server-function query while preserving typed operation/denial semantics; inbound MCP, SSE and full compatibility hosting stay deferred | Hermes/default Integration Authority Owner; bounded OMO module lanes implement disjoint carriers | active `PROC-011` still requires M00-authorized publication, durable restart and remaining black-box failure proof; peer-client stream/version conformance remains separate |
...
| `M40` Tool Gateway/Execution | `partial-evidence` | protocol/fake proof plus one bounded Affairs fixed first-party read-only adapter ordered behind current Market projection/recheck and in-memory Agent effect intent/receipt | durable intent/receipt journal, generic package-portable gateway and out-of-process executor host | unassigned | `AGENT-018/019`, `MARKET-007` |
...
| `M70` ChangeRadar | `partial-evidence` | exact-source-pinned semantic diff, digest-bound administrator approve/reject, coherent transaction-current M60 verification, exactly-once in-memory publication and deterministic Atom with stable GUID/provenance; no M00-authorized administration, durable restart or platform/Web composition | compose the reviewed change/feed through the bounded Market/Harness/ToolGateway path and Web board, then prove durable restart | Hermes/default Integration Authority Owner; bounded OMO audit lane | `RADAR-*` current-scope rows remain planned until composition/durability assertions pass |
...
| `M80` Client Core and Interaction Shells | `partial-evidence` | bounded client-core, real `ustc-agent affairs get/lookup` loopback JSON/exit path and one colocated operation-specific loopback Web presentation proof; no production auth/HTTP/stream, inbound MCP or Dioxus target | retain the least-privilege public-read boundary, then build the real peer Dioxus Web adapter over shared client semantics; generic conformance/Android/inbound MCP remain separate | Hermes/default Integration Authority Owner; bounded OMO module lane; Kimi K3 + Claude Opus 5 retain graphical UI/design ownership | active planned `PROC-011` thin-Web proof first; `CLIENT-*`, `WEB-*`, Android and deployment rows remain separate |
...
| `M70` ChangeRadar | bounded board-policy + typed semantic-diff + administrator review + coherent M60 verification + exactly-once in-memory Atom evidence (`partial-evidence`) | Market/Harness/ToolGateway + M10/Web board composition | durable source/baseline/review/feed restart | current three-plugin critical path |
