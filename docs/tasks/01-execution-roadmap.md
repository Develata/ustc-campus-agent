...
| `M30` Agent Harness/Runtime | `partial-evidence` | node kernel, provider-free deterministic Affairs/ChangeRadar turns and one bounded app-private three-turn/three-call Chat coordinator over M50 plus exact sequential tools; Opportunity consent/profile/planning semantics remain static M72 evidence rather than M30/M40 evidence | finite durable user-task harness/graph/context/review and generic journal composition | unassigned | `CHAT-001/002`; `HARNESS-*` + owned `AGENT-*` |
