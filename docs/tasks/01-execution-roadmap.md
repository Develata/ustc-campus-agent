...
|---|---|---|---|---|---|
| `M00` Platform Control/Identity | `partial-evidence` | identity-types, session-domain, bounded [`platform-request-context/v0`](../contracts/platform-request-context.md) (`AUTH-013`), B4a [`platform-session-port/v0`](../contracts/platform-session-port.md) (`AUTH-021`) and B4b [`platform-control-evidence/v0`](../contracts/platform-control-evidence.md) (`AUTH-022`) implemented; typed B4 interfaces/fakes are complete and B4a retains one secure durable current-session read vendor | one B5 administrator-admission composition path; production evidence persistence/transaction coupling stays with that accepted composition | unassigned | admitted/denied administrator request with stable redacted evidence and no downstream effect on denial |
| `M10` Application Ingress Host | `partial-evidence` | M10-owned client-protocol/application-ingress carriers plus one fixture-backed three-plugin loopback-Web composition; admitted Affairs, ChangeRadar and Opportunity calls pass through actor-aware invocation ports before bounded Market/Harness/ToolGateway/owning-Plugin paths, with no direct production fallback | replace fixture authority with M00-authorized administrator commands and production server-function queries while preserving typed operation/denial semantics; inbound MCP, SSE and full compatibility hosting stay deferred | Hermes/default Integration Authority Owner; bounded OMO module lanes implement disjoint carriers | bounded MVP rows have black-box HTTP/restart evidence; production administrator authority and peer-client stream/version conformance remain separate |
...
| `M70` ChangeRadar | `partial-evidence` | exact-source-pinned semantic diff, digest-bound administrator approve/reject, coherent transaction-current M60 verification, exactly-once in-memory publication and deterministic Atom now cross fixture-backed M10/current-Market/Harness/ToolGateway/owning-Plugin/loopback-Web JSON+Atom paths with disable/revoke, unknown-board and Affairs-isolation proofs; no M00-authorized administration or durable restart | replace fixture construction with M00-authorized administration and durable baseline/review/feed restart, then bind the remaining acceptance matrix | Hermes/default Integration Authority Owner; bounded OMO audit lane | `RADAR-*` current-scope rows remain planned until administrator/durability assertions pass |
...
Current bounded evidence covers the framework-neutral portions of B1/B2/B4/B7 for one Affairs operation and a fixture-backed loopback composition in `ustc-agentd`. It does not complete the operation registry, supported-version/upgrade matrix, B3 public server-function adapter, B5 stream, B6 lifecycle or B8 HTTP/SSE adapter; no batch is promoted beyond partial evidence by the loopback proof.
...
| `M70` ChangeRadar | bounded board-policy + typed semantic-diff + administrator review + coherent M60 verification + exactly-once in-memory Atom + fixture-backed Market/Harness/ToolGateway/M10/loopback-Web board composition (`partial-evidence`) | M00-authorized administration + durable source/baseline/review/feed restart | broader boards/subscriptions | bounded MVP composition complete; production administration remains |
