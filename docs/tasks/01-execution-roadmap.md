...
- `M30`: node-local `AgentRun` kernel with provider-backed and provider-free deterministic Harness turns;
- `M40`: Agent tool protocol/fake conformance plus bounded Affairs and ChangeRadar fixed-adapter composition proofs;
- `M72`: offline Course Planning plus bounded consent/private-profile foundation;
- `M90`: CI/checkers, app-private canonical-JSON persistence and one bounded loopback Compose package with deterministic archives, persistent restart/read-back and Windows launcher evidence; production infrastructure remains planned.
...
- `M40-B0`: `paused` — historical audit disposition AMEND; the later separately authorized static-M72 candidate resolves R2-F11 in current source without retroactively changing the W1 campaign disposition.
...
|---|---|---|---|---|---|
| `M00` Platform Control/Identity | `partial-evidence` | identity-types, session-domain, bounded [`platform-request-context/v0`](../contracts/platform-request-context.md) (`AUTH-013`), B4a [`platform-session-port/v0`](../contracts/platform-session-port.md) (`AUTH-021`) and B4b [`platform-control-evidence/v0`](../contracts/platform-control-evidence.md) (`AUTH-022`) implemented; fixed Affairs and ChangeRadar B5 commands persist/verify durable admission evidence before owning-product effects (`AUTH-023`/`AUTH-024`) | reusable production evidence vendor and formal auth integration; generic administration remains separate | unassigned | admitted/denied administrator request with stable redacted evidence and no downstream effect on denial |
| `M10` Application Ingress Host | `partial-evidence` | M10-owned client-protocol/application-ingress carriers plus one fixture-backed three-plugin loopback-Web composition and bounded deployment-local Chat/private-route access session; fixed Affairs/ChangeRadar administrator commands cross M00 durable evidence into owning ports, Affairs/ChangeRadar ordinary reads retain bounded Market/Harness/ToolGateway paths, and Opportunity's four private operations use the actor-aware static-M72 application port after transaction-current M20 authorization | production server-function queries and authenticated remote administration while preserving typed operation/denial semantics; inbound MCP, SSE and full compatibility hosting stay deferred | Hermes/default Integration Authority Owner; bounded OMO module lanes implement disjoint carriers | bounded MVP rows have black-box HTTP/restart evidence; production administration and peer-client stream/version conformance remain separate |
...
| `M30` Agent Harness/Runtime | `partial-evidence` | node kernel, provider-free deterministic Affairs/ChangeRadar turns and one bounded app-private three-turn/four-call Chat coordinator over M50 plus exact sequential tools; Opportunity consent/profile/planning semantics remain static M72 evidence rather than M30/M40 evidence | finite durable user-task harness/graph/context/review and generic journal composition | unassigned | `CHAT-001/002`; `HARNESS-*` + owned `AGENT-*` |
...
| `M50` Model Provider | `partial-evidence` | deterministic mock plus one bounded non-streaming operator-configured OpenAI-compatible Chat adapter with context preflight, typed errors and redacted file-backed secret handling | complete provider profiles, streaming/final parity, normalized real usage and full M30↔M50 conformance | unassigned | `CHAT-001` bounded evidence; provider conformance + real bounded turn remain planned |
...
| `M71` Affairs Navigator | `partial-evidence` | exact M60 `DemoReviewed` draft import, coherent M60 publication decision, digest-bound administrator approval and CAS/idempotent atomic publication compose with fixture-backed `Web/CLI → M10 → deterministic Harness → current Market authorization → ToolGateway → fixed Affairs adapter → M71`; durable command/idempotency records recover after restart, and disabled/revoked Market state denies before intent/executor/M60; startup fixture remains the bounded authority | replace startup fixture authority with an M00-authorized administrator command and production authority/publication repositories; broad search/refresh stays deferred | Hermes/default Integration Authority Owner; bounded OMO module lane | bounded MVP restart/E2E assertions pass; production administrator and temporal rows remain planned |
| `M72` Opportunity Graph | `partial-evidence` | deterministic planning now composes exact consent, per-request typed confirmation bound into the M10 digest, tenant-private durable profile/tombstone state, source/profile staleness, transaction-current M20 authorization with `Ask` fail-closed enforcement, one static M72 application use case per operation, typed projection and loopback Web create/view/plan/revoke-delete; real restart read-back and private-payload erasure are proved, with no Agent/provider/plugin execution spine | replace the DemoReviewed catalog and demo session with approved M60 import plus production M00/SSO and durable M20 authority; broader opportunity types remain deferred | Hermes/default Integration Authority Owner | bounded MVP composition/restart/deletion assertions pass; production source/identity and real-browser acceptance remain planned |
| `M80` Client Core and Interaction Shells | `partial-evidence` | bounded client-core and real `ustc-agent affairs get/lookup` loopback JSON/exit path; the colocated loopback chat-first three-plugin Web shell, local-access presentation and page-lifetime conversation are composition-only evidence credited to M10/M30/M50/M90 rather than M80; ChangeRadar and Opportunity do not yet use a generic M80 client core; no production auth/HTTP/stream, inbound MCP or Dioxus target | retain least privilege, then build the real peer Dioxus Web adapter over shared client semantics; generic conformance/Android/inbound MCP remain separate | Hermes/default Integration Authority Owner; bounded OMO module lane; Kimi K3 + Claude Opus 5 retain graphical UI/design ownership | bounded client-core and ordinary-user Affairs CLI evidence pass; peer-client, Android and deployment-productization rows remain separate |
| `M90` Infrastructure | `partial-evidence` | CI/checkers, app-private canonical-JSON persistence and one bounded loopback Docker Compose MVP package with deterministic archives, least-privilege runtime, operator-created verifier secret, persistent-volume restart/read-back and Windows launcher evidence | production config/store/journal/evidence adapters plus Fullstack backup/restore and public/Android deployment profile | unassigned | `CHAT-003` bounded Compose evidence; production restore/read-back remains planned |
...
- `M00-B3 request-context`: bounded platform-core implementation complete under `platform-request-context/v0` / `AUTH-013`: closed Public/Authenticated actor sum, immutable request-scoped descriptor, exact closed permission/effect surfaces and fail-closed coherence, current policy/session/capability checks, fenced idempotency, complete scalar disposition, persistence projection, fourteen rejections and exact 65-test evidence. B3 does not itself constitute B4 or B5 M10-v17 composition; the now-complete bounded B4 interface/fake evidence is recorded separately below.
- `M00-B4 ports-and-fakes`: B4a `session-port` (`platform-session-port/v0` / `AUTH-021`) and B4b `control-evidence` (`platform-control-evidence/v0` / `AUTH-022`) are bounded implemented as separate reviewable slices. They freeze exact session/evidence ports, deterministic fakes, one secure durable DemoReviewed current-session read vendor, stable redacted data-only event/error projections and append-once journal semantics. B4 is complete only as a typed interface/fake batch; production evidence persistence and atomic Affairs coupling remain B5/application-composition work.
- `M00-B5 api-admission-integration`: bounded fixed Affairs and ChangeRadar administrator commands attach to `M10`; admitted evidence is durable before the owning M71/M70 effect, and denied/evidence-failed requests reach no downstream mutation. Generic production administration remains planned.
...
Current bounded evidence covers the framework-neutral portions of B1/B2/B4/B7, fixed typed Affairs/ChangeRadar administrator commands, four static M72 private operations and a fixture-backed loopback composition in `ustc-agentd`. It does not complete the operation registry, supported-version/upgrade matrix, B3 public server-function adapter, B5 stream, B6 lifecycle or B8 HTTP/SSE adapter; no batch is promoted beyond partial evidence by the loopback proof.
...
- `M20-B5 invocation-authority`: bounded implementation is complete under `crate::market::authority`: one semantic carrier-by-carrier read transaction, service-owned candidate/current assembly, shared call preflight, adopted resolver/recheck and post-success precondition verification. The same owner now exposes a narrower static-application authorization for declarative resource components and it is consumed by the bounded M72 composition without constructing a tool projection. It creates no durable authority, production grant/enable evidence, effect intent or acceptance promotion.
...
- `M60-B2 retrieval-policy`: first bounded offline pure-policy implementation for exact host/path, redirects, content/time/size/rate, DNS/public-IP and response framing under the accepted `source-retrieval/v0` R11 packet; no transport port, clock, lease/journal, network effect, real-source approval or B3 authority.
...
`P1-1` remains the historical bounded `source-import/v0` review slice. The same `crates/platform-core/src/source_registry.rs` and `crates/platform-core/tests/source_registry.rs` carriers now implement bounded M60-B1 under `source-import/v1`; `SRC-001` remains `implemented` with the unchanged exact command `cargo test --locked -p ustc-campus-agent-core --test source_registry`, and the checker/test carriers close the exact v1 surface. `crates/platform-core/src/source_retrieval.rs` and `crates/platform-core/tests/source_retrieval.rs` retain the first bounded offline M60-B2 pure-policy algebra and synthetic conformance evidence; they implement no port, clock, lease/journal, network effect or real-source approval. `crates/platform-core/src/source_revision.rs` separately retains canonical-URL-bound, deterministic-ID immutable revision values for honestly labelled `DemoReviewed` snapshots plus an M60-owned revision-health type as supporting `M60-B5/B6` evidence. `M60` overall, every effectful B3+ retrieval/parser/durable-baseline/publication composition, and `SRC-010`, `SRC-011`, `SRC-012` remain `planned`; `SRC-014` remains catalog-only/non-admitted. The `ustc-teach-calendar-fall` candidate remains `Proposed`. The superseded V10 packet digest (`sha256:ba36425adc164ca9b3ec75addd4be2e4b299b5f8a8cfb75cf6a710679acd32ab`) is mechanically checked as historical evidence. Source approval, network retrieval and publication remain unauthorized.
...
Current bounded evidence implements parts of B1/B2/B5 plus a fixture-bound B4/B6 slice: checked draft/review/publication recovery records, one M00-authorized fixed administrator republish command, strict durable process restart, loopback operator CLI/HTTP/Web controls, and separate exact-query read-back through Market/Harness/ToolGateway. B3 supersession, generic administrator import/editor semantics, broad structured search, approved live M60 ingestion and the real peer Dioxus/M80 Web attachment remain open. Bounded `PROC-011` is implemented without promoting those later surfaces.

The first Affairs Navigator product slice follows the competition delivery posture (§18): administrator-imported reviewed snapshot → one typed procedure with bitemporal provenance (`valid_at` / `known_at`) plus review/verification metadata (`reviewed_at` / `last_verified_at`), with `as_of` bound as a query/answer cutoff ([`05-campus-trust-kernel.md`](../plan/05-campus-trust-kernel.md) §3.1) → one application query → one thin Web result with evidence/freshness/conflict/uncertainty. The `ustc-teach-calendar-fall` family remains an unapproved source candidate. The transcript/certificate page remains a source-grounded noncanonical `DemoReviewed` fixture with hash-accounted text/normalization, not an approved/live source or raw HTML archive; its exact imported draft/review/publication identity is now durably recovered only inside the bounded app-private adapter. `PROC-011` and `AUTH-023` are implemented active rows; `PROC-010` and `WEB-009` remain planned in their respective active/catalog scopes.
...
- `M72-B7 static M20 authorization plus M60/M10/M80 integration`; M30/M40/provider/plugin execution is not part of the four current private operations.
...
Current bounded evidence implements a deliberately smaller B1/B2/B4 subset for `affairs get/lookup` over the loopback fixture transport: one typed JSON result, stable exits, bounded timeout and no backend/operator implementation dependency. A separate colocated `ustc-agentd serve-web` proof renders one public-redacted `Available` result without capability/raw-revision exposure, but it does not use the shared core or Dioxus and therefore does not establish the peer Web/PWA target. The retained CLI surface must remain public/least-privilege and carry no secret/session/operator authority through argv; production profile auth, HTTP/TLS, NDJSON/SSE, reconnect/cancellation/version-skew, inbound MCP and every Dioxus target remain open.
...
- tag, GitHub Release, Pages, stable-download or public-runtime claims before release/public-readiness gates; the source repository itself is already intentionally public.
...
|---|---|---|---|---|
| `M00` Platform Control/Identity | identity/session/request-context + B4 ports and one fixed Affairs B5 admission/evidence composition (implemented as bounded evidence) | reusable production evidence vendor and formal auth integration | full SSO/multi-tenant lifecycle service | first production administrator surface |
| `M10` Application Ingress Host | framework-neutral query carriers + fixed typed Affairs administrator command and loopback HTTP/CLI/Web projection (`partial-evidence`; bounded `AUTH-023`/`PROC-011` implemented) | production-admitted server-function/public HTTP query route + dispatch/error projection; SSE remains optional | full client-contract + version-skew | first real peer Web adapter binding |
...
| `M30` Agent Harness/Runtime | node kernel plus one bounded app-private provider/tool Chat coordinator (`partial-evidence`; `CHAT-001`/`CHAT-002`) | durable finite user-task harness/graph over production authority | scheduler-supervisor + broad task orchestration | first durable Agent path beyond the competition Chat slice |
...
| `M50` Model Provider | deterministic mock plus one bounded non-streaming operator-configured OpenAI-compatible adapter (`partial-evidence`; `CHAT-001`) | complete typed provider profiles, streaming/final parity, normalized real usage and provider conformance | multi-provider peers | first production model-provider binding |
...
| `M72` Opportunity Graph | consent-bound tenant-private profile/planning/delete through M00/M10, transaction-current M20 authorization and static M72 application use cases with restart-persistent active/tombstone state (`partial-evidence`; no Agent/provider/plugin execution spine) | production M00/SSO + durable M20 authority + approved M60 course source | broader opportunity types and explanation | bounded MVP composition complete; real-browser and production identity/source remain |
| `M80` Client Core/Shells | bounded client-core + public-read `ustc-agent affairs get/lookup` loopback adapter (`partial-evidence`); the colocated Chat shell is composition-only M30/M50/M90 evidence, not M80 | real peer Dioxus Web adapter over shared client semantics | full Dioxus Web/Android + generic CLI/inbound MCP conformance | first peer Dioxus Web binding |
| `M90` Infrastructure | CI/checkers plus one bounded loopback Docker Compose MVP package with deterministic archives, least-privilege runtime, restart-persistent state and Windows launcher evidence (`partial-evidence`; `CHAT-003`) | production typed config/store plus Fullstack restore and migration | public/Android deployment profiles | first production deployment binding |
