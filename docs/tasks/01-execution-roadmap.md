...
|---|---|---|---|---|---|
| `M00` Platform Control/Identity | `partial-evidence` | identity-types, session-domain and bounded [`platform-request-context/v0`](../contracts/platform-request-context.md) kernel implemented (`AUTH-013`); production B4 ports/vendors and B5 M10 composition planned | durable port vendors and one M10-v17 composition path | unassigned | production crash/reopen/fencing plus admitted/denied API request proof |
| `M10` Application Ingress Host | `partial-evidence` | M10-owned client-protocol/application-ingress carriers plus fixture-backed Affairs and ChangeRadar TCP/loopback-Web compositions; admitted `affairs.get` and `change.list` calls pass through actor-aware invocation ports before bounded Market/Harness/ToolGateway/owning-Plugin paths, with no direct production fallback | replace fixture authority with M00-authorized administrator commands and durable server-function queries while preserving typed operation/denial semantics; inbound MCP, SSE and full compatibility hosting stay deferred | Hermes/default Integration Authority Owner; bounded OMO module lanes implement disjoint carriers | active product rows still require administrator authority, durable restart and remaining black-box failure proof; peer-client stream/version conformance remains separate |
...
| `M30` Agent Harness/Runtime | `partial-evidence` | node kernel plus provider-free deterministic Harness-turn path with replay and zero fabricated model usage | finite user-task harness/graph/context/review and durable journal composition | unassigned | `HARNESS-*` + owned `AGENT-*` |
| `M40` Tool Gateway/Execution | `partial-evidence` | protocol/fake proof plus bounded Affairs and ChangeRadar fixed first-party read-only adapters ordered behind current Market projection/recheck and in-memory Agent effect intent/receipt | durable intent/receipt journal, generic package-portable gateway and out-of-process executor host | unassigned | `AGENT-018/019`, `MARKET-007` |
...
| `M71` Affairs Navigator | `partial-evidence` | exact M60 `DemoReviewed` draft import, coherent M60 publication decision, digest-bound administrator approval and CAS/idempotent atomic publication now compose with fixture-backed `Web/CLI → M10 → deterministic Harness → current Market authorization → ToolGateway → fixed Affairs adapter → M71` queries over the same in-memory repository; disabled installation or revoked grant state denies before intent/executor/M60 with no direct fallback; no M00-authorized operator publication or durable adapter/restart | replace startup fixture authority with an M00-authorized administrator command, durable authority/runtime/publication repositories and restart read-back; broad search/refresh stays deferred | Hermes/default Integration Authority Owner; bounded OMO module lane | `PROC-010`/`PROC-011` remain planned until every temporal, administrator, restart and end-to-end assertion passes |
| `M72` Opportunity Graph | `partial-evidence` | deterministic Course Planning plus in-memory exact-consent, tenant-private profile, qualification/planning, source/profile staleness and revoke/delete evidence; no platform composition or durable private state | attach an authenticated consent/profile command and source-grounded planning operation through the shared Market/Harness/ToolGateway path, then prove restart/deletion semantics | Hermes/default Integration Authority Owner | `COURSE-*` current-scope rows; `COURSE-007`–`COURSE-009` remain planned until composition/durability assertions pass |
| `M80` Client Core and Interaction Shells | `partial-evidence` | bounded client-core, real `ustc-agent affairs get/lookup` loopback JSON/exit path and one colocated loopback Web shell with operation-specific Affairs plus ChangeRadar JSON/Atom presentation; ChangeRadar does not yet use M80 client-core; no production auth/HTTP/stream, inbound MCP or Dioxus target | retain the least-privilege public-read boundary, then build the real peer Dioxus Web adapter over shared client semantics; generic conformance/Android/inbound MCP remain separate | Hermes/default Integration Authority Owner; bounded OMO module lane; Kimi K3 + Claude Opus 5 retain graphical UI/design ownership | active planned product rows still require peer-client/durability proof; `CLIENT-*`, `WEB-*`, Android and deployment rows remain separate |
...
Current bounded evidence implements parts of B1/B2/B5 and a fixture-only B6 exact-query composition: stable IDs, typed draft/artifact/policy validation, freshness/conflict/uncertainty and one retained source-grounded noncanonical fixture reach M10, the ordinary-user client and a colocated loopback thin-Web renderer. B3 supersession, B4 real administrator review/publish/render, broad structured search, approved M60 ingestion and the real peer Dioxus/M80 Web attachment remain open.

The first Affairs Navigator product slice follows the competition delivery posture (§18): administrator-imported reviewed snapshot → one typed procedure with bitemporal provenance (`valid_at` / `known_at`) plus review/verification metadata (`reviewed_at` / `last_verified_at`), with `as_of` bound as a query/answer cutoff ([`05-campus-trust-kernel.md`](../plan/05-campus-trust-kernel.md) §3.1) → one application query → one thin Web result with evidence/freshness/conflict/uncertainty. The `ustc-teach-calendar-fall` family remains an unapproved source candidate. The current transcript/certificate page is retained only as a source-grounded noncanonical fixture with hash-accounted text/normalization; it is not an approved source, real M60 import, administrator publication or raw HTML archive. `PROC-010` (temporal/evidence semantics) and `PROC-011` (end-to-end first product result) are planned active acceptance rows for this slice; `WEB-009` remains a planned catalog case (catalog-only, not in `matrix.tsv`).
...
| `M10` Application Ingress Host | framework-neutral client-protocol/application-ingress carriers + fixture-backed loopback Affairs TCP and operation-specific HTTP/Web composition (`partial-evidence`; `PROC-011` remains `planned`/non-pass) | production-admitted server-function/public HTTP query route + dispatch/error projection; SSE remains optional | full client-contract + version-skew | first real peer Web adapter binding |
...
| `M70` ChangeRadar | bounded board-policy + typed semantic-diff + administrator review + coherent M60 verification + exactly-once in-memory Atom + fixture-backed Market/Harness/ToolGateway/M10/loopback-Web board composition (`partial-evidence`) | M00-authorized administration + durable source/baseline/review/feed restart | broader boards/subscriptions | current three-plugin critical path |
| `M71` Affairs Navigator | M60 `DemoReviewed` draft import + coherent source-health/retained-evidence decision + digest-bound review + bounded CAS/idempotent publication composed with same-repository fixture-backed M10/CLI/loopback-Web queries (`partial-evidence`) | M00-authorized operator publication + durable restart read-back (`PROC-011`) | broad tree/search/targeted refresh | `M60` reviewed snapshot + `M10` query |
...
| `M80` Client Core/Shells | bounded client-core + public-read `ustc-agent affairs get/lookup` loopback adapter + colocated operation-specific Web presentation proof (`partial-evidence`; `PROC-011` remains `planned`/non-pass) | real peer Dioxus Web adapter over shared client semantics | full Dioxus Web/Android + generic CLI/inbound MCP conformance | `M10` operation registry freeze |
