...
| `M71` Affairs Navigator | `partial-evidence` | hardened ProcedureDraft/artifact/policy/freshness/conflict kernel plus source-grounded noncanonical fixture, exact stable-ID composition and loopback thin-Web projection; no real source/admin lifecycle or production Web | complete the missing approved reviewed-snapshot import and administrator review/publish receipt without widening to broad search/refresh | Hermes/default Integration Authority Owner; bounded OMO module lane | `PROC-010`/`PROC-011` remain planned until every temporal, administrator and end-to-end assertion passes |
...
| `M71` Affairs Navigator | hardened ProcedureDraft/artifact/policy/freshness/conflict kernel + source-grounded noncanonical fixture + exact stable-ID query + loopback thin-Web projection (`partial-evidence`) | approved reviewed snapshot import + administrator review/publish (`PROC-011`) | broad tree/search/targeted refresh | `M60` reviewed snapshot + `M10` query |
