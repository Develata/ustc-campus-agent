...
| `M80` Client Core/Shells | bounded client-core + public-read `ustc-agent affairs get/lookup` loopback adapter + colocated three-plugin Web MVP (`partial-evidence`) | real peer Dioxus Web adapter over shared client semantics | full Dioxus Web/Android + generic CLI/inbound MCP conformance | `M10` operation registry freeze |
