...
- `Review`: GitHub Codex follow-up review `5077119247` on head `f0ea21447a8c7375b66ed45d6f2c085a1fd0bd7f` produced two further P2 findings: discussion `3903375281` identified invalid whitespace acceptance around a `Content-Type` parameter `=`, and discussion `3903375288` identified that the declared B2 projection-path tuple was not consumed by the owning checker.
- `Parser repair`: retain optional outer ASCII space around `;`, but stop trimming the split parameter name/value; `charset =utf-8` and `charset= utf-8` now fail `tchar`/quoted-value validation and have direct focused cases.
- `Projection repair`: every declared B2 projection carrier now has explicit required invariants; the exact declared path tuple is embedded inside the source-sensitive owning checker and must equal the ordered projection-map keys. A new sabotage mutates the authoritative trust-kernel plan into false transport/network authority and proves rejection. Checker inventory rises from `731` to `732` exact tests.
- `Repaired source hashes`: `source_retrieval.rs sha256:b91ed6cd3bda067e1185fc87ad7785bc1dfeb39e0fb1b90e78c279396792930b`; `tests/source_retrieval.rs sha256:14d95307bc271a822769c20b2016b1d68b9012916031449e24c7dbb0ea314db9`.
...
- `Review`: GitHub Codex review `5077549815` on head `171681b0da3f379f7f1ad612e10bee32f810ae75` found three exact false-green/representability gaps: marker-external task prose was not digest-bound; `SourceUrl` could admit a DNS host too long for `RetrievalDnsName`; and syntactically valid `Content-Length` above `u64::MAX` was misclassified as `AmbiguousFraming`.
- `Projection repair`: exact SHA-256 binding now covers every declared B2 projection carrier, including the immutable readiness proposal and the complete marker-external taskbook. Any additive contradiction changes bytes and fails closed; future legitimate taskbook edits require an explicit reviewed hash refresh.
- `DNS repair`: `SourceUrl` now admits canonical DNS hosts only through `253` presentation bytes, preserving its existing `1..=63` label rule. External registry tests accept the exact `253` boundary and reject a valid-label `255`-byte host as `InvalidHost`, so every current approved URL host is representable as `RetrievalDnsName`.
- `Content-Length repair`: malformed decimal syntax remains `AmbiguousFraming`; syntactically valid decimal overflow is `DeclaredBodyTooLarge`. Integration tests bind both `01` and `18446744073709551616` classifications.
- `Frozen Rust bytes`: `source_registry.rs=3504fe9c470e123cf87af894fab37bb1095f33ba753b01818b9d16d9238e0a64`; `tests/source_registry.rs=167510be70ff1a0ff019efeb7f6ab2b98ac815ffa15ab0ac4004f0f42d8393f2`; `source_retrieval.rs=afe0ced332d7cfcb3728cf320e9aeb4150f1619528b6036e4b5271b4980b0a16`; `tests/source_retrieval.rs=232b31e299bb888ddac76d6471c553a3f8e7f5c529ece043769985e1023f6abd`.
...
- `Review`: GitHub Codex review `5077875329` on head `5f2fcd0d4af45ee3440250d65eb9f35d4964bae1` identified malformed-head precedence, missing bounded B2 acceptance/feature binding and stale module-map review metadata. A paired exhaustive packet `sha256:f80054b211e67f6f4866d250686c0cdf577e7a506004cbdb554d75836bc84585` produced mandatory Codex `BLOCKED` result `sha256:6dea194d986639471d278b956524af8d2d22f7047708e3b0b5f60e119f474edf` and independent DeepSeek `PASS` result `sha256:d4de83c96a406bec0fc3ad0203225397ce3aa05e015c4790276fc574d99692ff`; Main Hermes independently accepted the three repository findings and adjudicated the response `Connection: close` concern as contract clarification because the exact serialized request already fixes that header.
- `Repair`: strict response-head parsing now classifies malformed grammar before the raw-byte cap and covers malformed/valid oversized combined cases; close-delimited framing is explicitly request-scoped and tested; implemented `SRC-015` binds the pure offline B2 algebra in `matrix.tsv`, the long-horizon catalog and ChangeRadar feature projection without promoting effectful `SRC-010`; module-map `Last Review` is `2026-09-01`.
- `Frozen Rust identities`: `source_registry.rs=3504fe9c470e123cf87af894fab37bb1095f33ba753b01818b9d16d9238e0a64`, `tests/source_registry.rs=167510be70ff1a0ff019efeb7f6ab2b98ac815ffa15ab0ac4004f0f42d8393f2`, `source_retrieval.rs=6cbbcd5277fcfe99389735dd1b97879b77267710cef5421894df80d9ee93b5e8`, `tests/source_retrieval.rs=d52cba008bfc9bd271d6bea776ba0822086bd598f24355a185a27caeda5ab31f`.
- `Required closure`: focused remote Rust source gates, complete checker shards including one additive acceptance-binding sabotage, exact-head CI/governance and final blocker-only formal review must pass before merge authorization is requested.
- `Non-claims`: `M60` overall and `SRC-010/011/012` remain `planned`; `SRC-014` remains catalog-only; there is no network effect, transport port, approved source, B3/M90 implementation, deployment or merge authority.

## Marker-external final-review repair R10

- `Review`: GitHub Codex review `5078233804` on head `c72ceb7056a6217b80091313099c45d64b1c3f54` found that raw-size admission still preceded malformed status/header grammar and that the M60 plan's old no-`SRC-*`-promotion sentence contradicted implemented bounded `SRC-015`.
- `Repair`: a complete no-limit grammar preflight now validates the status line and every header name/value before any raw/name/value/count limit result; combined oversized malformed status/header cases prove `MalformedResponseHead`. The plan now explicitly promotes only non-effectful `SRC-015` while preserving `M60` overall and effectful `SRC-010/011/012` as `planned` and `SRC-014` as catalog-only.
- `Required closure`: focused remote Rust source gates, complete `734`-test checker shards, exact-head CI/governance and final blocker-only review must pass before merge authorization is requested.
- `Non-claims`: no network effect, transport port, approved source, B3/M90 implementation, deployment or merge authority.
...
- `Pull-request repair R4 source gates`: `PASS`; controller-owned receipt `sha256:5684a919302dc46af164648093ed31df376b6845d0cc77ffc22efcfc2c012500`; focused format/check/tests/doc/clippy remained green after the parameter-OWS and projection-checker repairs, exact-wire sabotage still bit with exit `101`, and post-restore source/test hashes matched `b91ed6cd3bda067e1185fc87ad7785bc1dfeb39e0fb1b90e78c279396792930b` / `14d95307bc271a822769c20b2016b1d68b9012916031449e24c7dbb0ea314db9`.
- `Pull-request repair R7 source gates`: `PASS`; controller-owned receipt `sha256:70d30b20358be3967b242deabc7cb7251284a03a348ad245e6aa87d9a845c634`; focused format/check/source-registry/source-retrieval/unit/doc/clippy gates passed for all four frozen Rust carriers, exact-wire sabotage bit with exit `101`, and post-restore hashes matched the R7 identities above.
- `Pull-request repair R9 source gates`: `PASS`; controller-owned receipt `sha256:21a80de3a8763d5f4cb36a6aaca3d07d13924ff61722083d01cbb8ef63f75b3d`; focused format/check/source-registry/source-retrieval/unit/doc/clippy gates passed for all four frozen Rust carriers, exact-wire sabotage bit with exit `101`, and post-restore hashes matched the R9 identities above.
- `Pull-request repair R10 source gates`: `PASS`; controller-owned receipt `sha256:c2da2fa3e1683d71e4ce535fbb41753f75ba97f445a3717124db5c9d72ba1b7f`; focused format/check/source-registry/source-retrieval/unit/doc/clippy gates passed for all four frozen Rust carriers, exact-wire sabotage bit with exit `101`, and post-restore hashes matched the R10 identities above.
