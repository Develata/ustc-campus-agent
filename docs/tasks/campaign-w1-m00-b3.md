...
Produce one source-bound readiness packet that identifies the missing request-context and policy-reference acceptance rows, exact actor/policy/admission choices, owned-path proposal and checker/test obligations. The packet may be pushed as a Draft PR but MUST pause for Develata before accepting authority semantics or retaining implementation.

## Required evidence

- exact source commit and clean checkout receipt;
- current blueprint/contract/acceptance gap table;
- proposed acceptance IDs and future bindings without promotion to `implemented`;
