...
- `Stage`: `PROPOSAL_ONLY_R61_CANDIDATE_AWAITING_DELTA_REVIEW`
- `Disposition`: `R60_GITHUB_BLOCKERS_REPAIRED_LOCALLY_R61_UNREVIEWED`
...
- `Packet digest`: `sha256:77bc724bca83c2922637e73c5d2c351eee9dce7828339f3cff6ab1d5b589071f` over `213504` bytes beginning immediately after the packet `BEGIN` marker newline and ending immediately before the `END` marker token, including the final packet newline
- `Candidate generation`: `R61`
- `Current blockers`: exact-head GitHub Codex found three construction blockers on committed R60; all three are repaired in this local R61 semantic candidate, but no R61 independent verdict exists yet
- `Next allowed mutation`: freeze R61 plus the exact R60-to-R61 delta, run mandatory Codex and one heterogeneous delta-binding review, replay scoped proposal gates, then bind a marker-external receipt without changing packet bytes
- `Remote posture`: R60 remains exact remote PR #66 head `9435542e13689f7415d890748a3631add3d651df`; exact-head `rust`, `docs-and-contracts` and `ci-governance` passed, but three unresolved exact-head GitHub Codex P1 threads block merge. R61 is local and unreviewed. No source-status mutation, live retrieval, DNS/socket/HTTP, retained Rust implementation, deployment, release or publication is authorized by this proposal
...
OperationDescriptorProjectionV1
  ...
  required_platform_operator_capability_id() -> Option<&CapabilityId>
...
PersistedAdmissionRejectionDtoV1 {
  projection: exact validated durable mirror of AdmissionRejectionProjectionV1,
  rejection_digest: Sha256Digest
}
...
AdmissionPortsV1
  ...
  resolve_platform_operator_grant(admitted_user_id, required_capability_id,
                                  operation_id, schema_digest,
...
4. Descriptor: `enc(operation_id) || enc(schema_identity) || schema_digest[32] || enc(permission_class.wire_tag()) || enc(effect_class.wire_tag())`, then required platform-operator capability as one byte `0x00` with no following field or `0x01 || enc(required_capability_id)`, then `enc(decoder_identity) || enc(dispatcher_identity) || u32_be(adapter_count)`, followed by each `enc(adapter_identity)` in retained descriptor order, then `enc(descriptor_snapshot_id)`.
...
`PersistedAdmissionRejectionDtoV1.rejection_digest` is SHA-256 over ASCII domain `"platform-request-context/v1/rejection\0"`, followed by `enc(projection.kind.wire_tag())` and every displayed field of that exact `AdmissionRejectionProjectionV1` variant in declaration order. Textual IDs plus `permission_class`, `actor_kind` and `port` exact wire tags use §6.1 `enc`; `observed_at` uses checked big-endian `i128`; `MalformedCommand.operation_id` uses byte `0x00` when absent or `0x01 || enc(operation_id)` when present. No field from another variant is admitted and the digest itself is excluded. At rejected finalization the idempotency boundary derives the durable projection from `FinalAdmissionDispositionV1::Rejected`, computes this digest itself and atomically persists both; callers cannot supply either durable carrier or digest. Before either `PriorIdentical` or `PriorWon`, and again before `promote_persisted_prior_v1` returns `PriorRejected`, it recomputes the digest from the complete decoded projection and requires exact equality. A well-typed one-axis field mutation, variant/tag substitution, unknown/extra field, invalid scalar or digest mismatch is `CorruptState` before any prior content is projected.

The v1 admission/rejection/idempotency family is closed as one versioned persistence boundary. `AdmissionRejectionProjectionV1` and `PersistedAdmissionRejectionDtoV1.projection` have the same fourteen variant field lists displayed above; their exact explicit `kind` tags are the accepted logical tags, but the durable DTO is decoded only inside `PersistedPriorDispositionEnvelopeV1` with the exact v1 schema identity and `deny_unknown_fields`. `PolicyDenied.permission_class` is `PermissionClassV1` in both live and persisted forms. Every prior envelope retains the exact original envelope basis; an admitted prior additionally retains the complete original canonical admission-binding preimage in `PersistedAdmissionBindingDtoV1`. `PersistedActorReferenceDtoV1` is derived exactly as `M00AdmittedActorV1::Public -> Anonymous` and `Authenticated { tenant_id, user_id, session_id } -> Authenticated { session_id }`; no tenant/user field enters envelope identity. The nested actor-reference, operation, admitted-actor and operator-admission DTOs have exactly the displayed fields, preserve adapter order, deny unknown fields and use the same checked scalar/enum decoding as the live v1 carriers.
...
Both new permissions are private platform-operator permissions: an anonymous actor or an incoherent descriptor is `MalformedCommand { operation_id: Some(current_operation_id) }`. The validated descriptor must expose `required_platform_operator_capability_id = None` for each inherited v1 permission/effect pair and `Some(exact CapabilityId)` for `PlatformOperatorWrite | PlatformOperatorEffect`; the inverse presence rule, an empty/unknown ID, or the wrong exact operation-to-capability mapping is a descriptor mismatch before authority lookup.
For the four inherited pairs, M00 uses `check_capability` exactly as accepted v0 and requires `platform_operator_admission = None`. For either platform-operator permission, only an admitted authenticated actor may proceed; M00 passes the exact required capability from that validated descriptor into `resolve_platform_operator_grant` after descriptor, policy and session currentness. Its closed mapping is `Missing -> CapabilityMissing`, `Disabled | NotYetActive -> CapabilityDisabled`, and `Revoked | Expired -> CapabilityRevoked`.
`Granted(snapshot)` is admitted only when `snapshot.admitted_user_id` equals the admitted actor, `snapshot.capability_id` equals the required capability argument, operation/schema/policy IDs equal the current validated descriptor and policy, `grant_state == Active`, and `grant_not_before <= observed_at < grant_not_after`; a malformed range or any capability/identity/state/time disagreement in `Granted` is an impossible adapter observation mapped to `InfrastructurePortUnavailable { operation_id, port: Capability }` with `diagnostic_source: Port(Capability)`. M00 then owner-mints `PlatformOperatorAdmissionV1` by copying the checked snapshot and the one trusted `observed_at`; neither callers nor DTO decoders can construct it.
...
RequiredCapabilityId: platform.source.approve
...
RequiredCapabilityId: platform.source.retrieve
...
The parser also requires `platform-request-context/v1` to declare the distinct full `PermissionClassV1`/`EffectClassV1` inventories above, preserve accepted v0 declarations byte/API exact, add only the four v1-only values and two new coherence pairs, reproduce the fourteen rejection classes/mappings through the distinct v1 rejection/persistence family with the exact canonical rejection digest, expose each descriptor's exact optional required-capability field plus the checked platform-operator grant snapshot/disposition resolver argument and returned-snapshot capability equality, and bind each v1 descriptor to the M10/M60 executable consumer; naming a descriptor without the separate v1 enum/wire declarations, omitting or mismatching its required capability, or widening a v0 enum fails.
...
`RawSnapshotWriteReceipt` is the displayed public data-only five-field metadata value. Its M60-owned `write_receipt_digest` is exactly SHA-256 over ASCII domain `"ustc-raw-source-snapshot-write-receipt/v1\0"`, followed by `enc(snapshot_identity) || enc(source_id) || enc(attempt_id) || u64_be(body_byte_count) || body_sha256[32]`. The five-field receipt carries no digest field: after `Stored | AlreadySame`, M60 first requires snapshot/source/attempt identity to equal the exact requested/retained values, body count and digest to equal the bounded bytes supplied to `put_if_absent` and the retained successful transport metadata, then computes the digest itself from those validated fields. Any field disagreement, invalid count/digest, unknown/extra receipt field or preimage mismatch is `RepositoryCorrupt`; it starts no read-back, mints no `RawSnapshotStored` or completion decision, releases no slot and leaves `TransportStopped` recoverable. `Stored` and `AlreadySame` use the same canonical preimage and can differ only in the outer put-result tag, which is deliberately not part of the receipt digest.
`RawSnapshotReadObservationV1` is the corresponding M60-consumed read carrier and additionally owns `BoundedRawSnapshotBytesV1` plus the exact bound used for that read. The byte carrier is move-only, has private fields and no Serde/`Clone`/`Default` or unchecked constructor; its sole public data-only constructor is `BoundedRawSnapshotBytesV1::try_from_bytes(bytes: Box<[u8]>, maximum_response_bytes: u32) -> Result<Self, BoundedRawSnapshotBytesErrorV1>`, which performs checked `u64::try_from(bytes.len())` and requires the result not exceed the bound. Its only accessors are `as_bytes()`, checked `len_u64()` and consuming `into_boxed_bytes()`. M60 uses that constructor for the successful transport body before `put_if_absent`; a conforming raw adapter uses it before returning `Found`. Initial read-back derives/passes the current sealed request limit; recovery derives/passes the attempt row's retained transaction-current `maximum_response_bytes`; no external caller supplies either value. Constructor failure maps to the owning path's existing nonterminal `RepositoryCorrupt` and performs no raw/journal/release mutation.
...
`InitialWriteReadback` additionally requires the current `InitialAfterTransport` authorization plus the exact validated `Stored | AlreadySame` write receipt and binds the M60-recomputed canonical `write_receipt_digest` above.
...
Gate before R5. Exact executable R1/R3/R4 tests must pass for: every adjacent
    ...
    from the sealed context plus the reduced disposition; rejected finalization instead
    persists the complete projection plus M00-computed canonical `rejection_digest`;
    one-axis omission/mutation of
    ...
    parity plus recomputed digest equality; one-axis mutation of every envelope basis,
    Public/Authenticated actor projection,
    nested admitted actor, descriptor, adapter order, policy/time, provenance, operator
    grant, payload, digest, schema, frozen prerequisite or rejected tag/field—including
    a different still-valid `PolicyExpired.policy_snapshot_id` and
    `SessionNotAdmitted.observed_at` without digest repair—returns
    ...
    the exact descriptor-required capability is passed into the resolver and a valid exact
    active/window-current snapshot mints `PlatformOperatorAdmissionV1`, while one-axis
    user/capability/operation/schema/policy/state/window/range mismatches in Granted
    ...
    exact five-field `RawSnapshotWriteReceipt`, canonical M60-owned write-receipt digest
    domain/field order and checked-only bounded-byte carrier declaration compile; Stored and
    AlreadySame produce the same digest for the same five fields, each one-axis receipt-field
    mutation or retained-body mismatch returns nonterminal `RepositoryCorrupt` before
    read-back/decision/release, constructor success/length-overflow/limit-exceeded, accessor
    and consuming paths are covered, and any unchecked/default/Serde/Clone construction fails;
...
### R61 exact-head construction repair

R61 supersedes R60 after exact-head GitHub Codex review of commit `9435542e13689f7415d890748a3631add3d651df` found three construction blockers. It changes only the three cited risk surfaces: rejected persisted dispositions now carry an M00-computed digest over every variant field and must pass complete recomputation before either prior outcome; platform-operator descriptors carry the exact required capability into grant resolution and reject a wrong-capability `Granted` snapshot; and the unchanged five-field raw-snapshot write receipt now has one M60-owned canonical digest preimage validated before read-back or terminalization. R61 neither widens authority nor changes source/network posture. All R60 semantic/review receipts remain historical evidence for their exact older bytes and do not approve R61.
