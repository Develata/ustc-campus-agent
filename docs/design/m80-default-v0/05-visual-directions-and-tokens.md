...
> 覆盖 artifact：12 Design tokens（两套方向 + 比较 + 推荐）
> 对比度数字为 WCAG 2.x 相对亮度公式实际计算值（公式与结果表见 §6），非肉眼判断。所有 token 数值为 `PROPOSED_DESIGN_TOKEN` 起点，Stage B 以真实 browser/device 测试冻结。
...
## 2. Direction B — Student Field Notes
...
### 2.4 B 的非色彩差异（coherent direction ≠ 换 palette）
...
## 3. 两方向比较与推荐（comparison memo）

| 维度 | A Quiet Evidence | B Field Notes |
|---|---|---|
| 信任/权威场景（approval、permission、audit） | 强：低刺激、证据优先 | 风险：暖调 + 口语 microcopy 可能削弱严肃感 |
| Today/校园工具的亲和力 | 中：需 microcopy 补暖 | 强：笔记隐喻贴合学生心智 |
| 长阅读/diff/表格 | 强 | 中强 |
| 与“非官方但可信”的定位 | 一致：克制即诚实 | 部分一致：需严格限定使用面 |
| 实现复杂度 | 一套系统 | 两套语义规则，混合风险高 |

**推荐（PROPOSAL）**：**A 为 core system**；允许 B 的 warmth（纸色、口语 microcopy、editorial illustration）只进入 empty states、onboarding 与非权威 editorial 区。Approval、permission、audit、diff、receipt 一律 A。
...
## 4. 共享 token envelope（两方向同构，仅色值不同）

### 4.1 Type（Chinese-first system sans）
...
### 4.2 Measure / spacing / grid / radius / border / shadow
