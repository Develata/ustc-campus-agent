...
- round-7 repair（2026-08-06，仅 16B prototype；checker/tests/CI 冻结未动）：**edge hardening**——`.row .meta`/`.note`/`.warn`/`.ok`/`label.ck .meta` 增加 `overflow-wrap:anywhere`；320px 注入 600 字符 ASCII 极端值实测无横向溢出（16 屏 + S12-confirmed/S11-stale 分支全覆盖，14 组全过）；不动 PR body。
