...
- `platform-core` also carries bounded **M60** evidence: `source_registry.rs` owns the pure `source-import/v1` B1 lifecycle, while `source_retrieval.rs` owns only the first offline `source-retrieval/v0` B2 policy/observation algebra. The latter must remain standard-library-only and I/O-free: no port implementation, runtime, clock, lease/journal, network effect, concrete-source approval or B3 carrier. M60 overall and `SRC-010/011/012` remain `planned`; `SRC-014` remains catalog-only.
...
The retained M60-B2 surface in `crates/platform-core/src/source_retrieval.rs` is a pure offline policy kernel over synthetic/caller-supplied observations. It must not acquire transport, DNS, socket, HTTP, filesystem, journal, clock, source-approval or B3 admission authority without a separately reviewed contract and implementation slice.
