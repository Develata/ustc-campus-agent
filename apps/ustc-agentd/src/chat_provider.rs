...
fn optional_json_text<'a>(value: &'a Value, field: &str) -> Option<Option<&'a str>> {
    match value.get(field)? {
        Value::Null => Some(None),
        Value::String(text) if !text.trim().is_empty() => Some(Some(text)),
        _ => None,
    }
}

fn summarize_affairs_result(data: &Value) -> Option<String> {
    if json_text(data, "/kind")? != "available"
        || json_text(data, "/terminal/outcome/kind")? != "found"
    {
        return None;
    }
    ...
    let steps = view.pointer("/ordered_steps").and_then(Value::as_array)?;
    let entry_points = view.pointer("/entry_points").and_then(Value::as_array)?;
    let mut summary = format!("办事流程：{title}\n流程编号：{procedure_id}");

    if steps.is_empty() {
        summary.push_str("\n当前 reviewed procedure 未列出办理步骤。");
    } else {
        summary.push_str("\n办理步骤：");
    }
    for (index, step) in steps.iter().enumerate() {
        let ordinal = step.get("ordinal").and_then(Value::as_u64)?;
        let instruction = json_text(step, "/instruction")?;
        if index < 6 {
            summary.push_str(&format!("\n{ordinal}. {instruction}"));
        }
    }
    if steps.len() > 6 {
        summary.push_str(&format!("\n另有 {} 个步骤未展开。", steps.len() - 6));
    }
    ...
    for (index, entry) in entry_points.iter().enumerate() {
        let label = json_text(entry, "/label")?;
        let url = optional_json_text(entry, "url")?;
        if index < 3 {
            summary.push_str(&format!("\n- {label}：{}", url.unwrap_or("未提供链接")));
        }
    }
...
fn summarize_change_result(data: &Value) -> Option<String> {
    if json_text(data, "/kind")? != "change_feed_accepted"
        || json_text(data, "/terminal/outcome/kind")? != "found"
    {
        return None;
    }
    ...
    for (entry_index, entry) in entries.iter().enumerate() {
        let scope = json_text(entry, "/affected_scope")?;
        let fields = entry.pointer("/changed_fields").and_then(Value::as_array)?;
        let source_url = json_text(entry, "/source_url")?;
        if fields.is_empty() {
            return None;
        }
        if entry_index < 3 {
        ...
        for (field_index, field) in fields.iter().enumerate() {
            let name = json_text(field, "/field")?;
            let before = optional_json_text(field, "before")?;
            let after = optional_json_text(field, "after")?;
            if entry_index < 3 && field_index < 6 {
                summary.push_str(&format!(
                    "\n- {name}：{} → {}",
                    before.unwrap_or("—"),
                    after.unwrap_or("—")
...
fn summarize_opportunity_result(data: &Value) -> Option<String> {
    if json_text(data, "/kind")? != "opportunity_accepted"
        || json_text(data, "/terminal/kind")? != "plan_generated"
    {
        return None;
    }
    ...
    match json_text(decision, "/kind")? {
        ...
        "planned" => {
            ...
            let global_violations = decision
                .get("hard_constraint_violations")
                .and_then(Value::as_u64)?;
            let warnings = decision.pointer("/warnings").and_then(Value::as_array)?;
            if candidates.is_empty() || warnings.iter().any(|warning| json_text(warning, "").is_none())
            {
                return None;
            }
            ...
            summary.push_str(&format!(
                "\n全局 hard-constraint violations：{global_violations}"
            ));
            for (index, candidate) in candidates.iter().enumerate() {
                let course_code_values = candidate
                    .pointer("/course_codes")
                    .and_then(Value::as_array)?;
                let course_codes = course_code_values
                    .iter()
                    .map(|value| json_text(value, ""))
                    .collect::<Option<Vec<_>>>()?;
                let total_credits = candidate.get("total_credits").and_then(Value::as_u64)?;
                let soft_score = candidate.get("soft_score").and_then(Value::as_i64)?;
                let hard_violations = candidate
                    .pointer("/hard_constraint_violations")
                    .and_then(Value::as_array)?;
                let hard_violation_text = hard_violations
                    .iter()
                    .map(|value| json_text(value, ""))
                    .collect::<Option<Vec<_>>>()?;
                let rationale = candidate.pointer("/rationale").and_then(Value::as_array)?;
                let rationale_text = rationale
                    .iter()
                    .map(|value| json_text(value, ""))
                    .collect::<Option<Vec<_>>>()?;
                candidate
                    .pointer("/requirement_credits")
                    .and_then(Value::as_array)?;
                candidate.pointer("/provenance").and_then(Value::as_array)?;
                if course_codes.is_empty() {
                    return None;
                }
                if index < 3 {
                    summary.push_str(&format!(
                        "\n候选 {}：{}（{total_credits} 学分，soft score {soft_score}）",
                        index + 1,
                        course_codes.join("、")
                    ));
                    if hard_violation_text.is_empty() {
                        summary.push_str("；hard constraints 通过");
                    } else {
                        summary.push_str(&format!(
                            "；hard-constraint warnings：{}",
                            hard_violation_text.join("；")
                        ));
                    }
                    for line in rationale_text.iter().take(6) {
                        summary.push_str(&format!("\n- {line}"));
                    }
                    if rationale_text.len() > 6 {
                        summary.push_str(&format!(
                            "\n- 另有 {} 条理由未展开。",
                            rationale_text.len() - 6
                        ));
                    }
                }
            }
...
fn summarize_calendar_result(data: &Value) -> Option<String> {
    if json_text(data, "/schema")? != "ustc-simple-calendar-result/v1"
        || json_text(data, "/package_id")? != "ustc.simple-calendar"
    {
        return None;
    }
    match json_text(data, "/action")? {
        ...
        "delete" => {
            ...
            let item_id = json_text(item, "/id")?;
            let title = json_text(item, "/title")?;
            optional_json_text(item, "scheduled_for")?;
            item.get("created_at_unix_secs").and_then(Value::as_u64)?;
...
#[cfg(test)]
#[path = "chat_provider/summary_tests.rs"]
mod summary_tests;
