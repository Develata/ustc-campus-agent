...
if [ -n "$(git -C "$repo_root" status --short --untracked-files=no)" ]; then
  printf 'all tracked package and binary source paths must be clean\n' >&2
  exit 65
fi
if [ -n "$(git -C "$repo_root" status --short --untracked-files=all -- \
  Cargo.toml Cargo.lock rust-toolchain.toml .cargo apps crates build.rs)" ]; then
  printf 'untracked package binary build inputs are forbidden\n' >&2
  exit 65
fi
