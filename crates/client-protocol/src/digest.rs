...
const OPPORTUNITY_DOMAIN: &[u8] = b"m10/opportunity/payload-digest/v1\0";
...
pub fn opportunity_payload_digest(
    ...
    WireText::parse(hex_lower(&hasher.finalize())).map_err(|_| OpportunityDigestError::Encoding)
}
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum OpportunityDigestError {
    Command,
    Length,
    Encoding,
}

impl std::fmt::Display for OpportunityDigestError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        formatter.write_str(match self {
            Self::Command => "Opportunity payload is invalid",
            Self::Length => "Opportunity payload digest input length overflow",
            Self::Encoding => "Opportunity payload digest encoding failed",
        })
    }
}

impl std::error::Error for OpportunityDigestError {}
...
mod tests {
    ...
    fn opportunity_digest_is_canonical_and_domain_separated() {
        ...
        let command = OpportunityCommandDto::CreateProfile {
            ...
            consented_at: UnixMillis::new(1),
            completed_courses: vec![proc_id("MATH1002"), proc_id("MATH1001")],
            min_credits: 9,
            max_credits: 12,
            preference_weights: vec![OpportunityPreferenceDto {
                course_code: proc_id("MATH2001"),
                weight: 9,
            }],
        };
        let digest =
